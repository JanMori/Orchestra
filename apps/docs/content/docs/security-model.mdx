...
description: What a Orchestra task can reach on the machine that runs it, and where the real isolation boundary lives.
---
...
Orchestra does not sandbox the filesystem for you. If the daemon runs as your personal user account, a task can read your SSH keys, edit your shell profile, and delete your documents. Isolation has to come from the boundary you put the daemon in.
</Callout>
...
So Orchestra does not pretend to be the boundary. Put one around it.

## Recommended setups

Pick whichever fits your infrastructure — they are listed from lightest to strongest:

1. **Dedicated Unix user.** Create a `orchestra` user, give it only the repositories and credentials the agents need, and run the daemon as that user. Your own account stays untouched.
...
- **Per-task working directory.** Each task gets its own workdir under `~/orchestra_workspaces/`, so concurrent tasks do not collide on the same checkout.
...
- **Task-scoped API tokens.** The `ORCHESTRA_TOKEN` handed to a task is bound to that agent and that task by the server, so a task cannot act as you or as another agent through the Orchestra API.

## What is not a boundary

- **The coding tool's own sandbox and approval settings.** Orchestra runs agents unattended, so approval prompts are answered automatically. On the default path the filesystem sandbox is off as well: Codex runs with `sandbox_mode = "danger-full-access"` and Claude Code with `--permission-mode bypassPermissions`. The exception is Windows — if you explicitly configure Codex's native sandbox (`windows.sandbox = "unelevated"` or `"elevated"`), Orchestra honors that opt-in and keeps `workspace-write` for those tasks. Worth doing where it fits, but it is one tool on one platform and it does not change how you should scope the daemon user.
...
```bash
orchestra daemon logs --lines 200 | grep "codex sandbox"
```

To confirm the effective Codex configuration, read the managed block in the task's `config.toml` under the task's `CODEX_HOME` — the section between the `# BEGIN orchestra-managed` and `# END orchestra-managed` markers is written by the daemon on every run.
