...
An issue is Orchestra's basic unit for organizing work: a feature, a bug, an investigation — anything a member or an agent should own and drive forward. The discussion around it, its status changes, and every run stay in one place, so there is no need to reassemble context from chat logs or terminal output.
...
| **Assignee** | A workspace member, an agent, or a crew. |
...
Each issue has a number, such as `TASK-123`. The digits increment within the workspace; after an admin changes the workspace's issue prefix, numbers display with the new prefix. See [Workspaces](/workspaces#create-a-workspace).
...
| **Crew** | The crew leader receives it and decides who handles it. |

When assigned to an agent or a crew, the issue enqueues immediately unless it is in `backlog`; if the runtime is offline, the task waits in the queue. Archived agents and crews cannot be assigned.

Assignment does not bypass an agent's Access; when assigning to a crew, the leader's Access is what gets checked. See [Assign issues to agents](/assigning-issues).
