...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `ORCHESTRA_SERVER_URL` | `ws://localhost:8080/ws` | Multica API / WebSocket 地址，也接受 `http(s)` |
...
| `ORCHESTRA_DAEMON_AUTO_RELOAD` | `true` | 是否在 `multica` 二进制被带外替换（`brew upgrade`、重新下载、本地构建）后重启到新二进制。与 `ORCHESTRA_DAEMON_AUTO_UPDATE` 相互独立 |
| `ORCHESTRA_WORKSPACES_ROOT` | `~/multica_workspaces` | 执行任务工作目录的根目录 |
