...
설치 후 Web 버전과 같은 이메일로 로그인합니다. 로그인하면 Desktop이 내장 Orchestra CLI를 시작하고 로컬에 설치된 AI 코딩 도구를 감지합니다.

<Callout type="info">
Desktop 내장 CLI는 앱이 런타임을 관리할 때만 사용합니다. 터미널에서 `orchestra issue` 같은 명령도 실행하려면 [빠른 시작 2단계](/cloud-quickstart#2-컴퓨터-연결)에 따라 CLI를 별도로 설치하세요.
</Callout>
