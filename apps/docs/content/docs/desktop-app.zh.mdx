...
安装后使用和 Web 版相同的邮箱登录。登录后，Desktop 会启动自己内置的 Orchestra CLI，并检测本机已有的 AI 编程工具。

<Callout type="info">
Desktop 内置的 CLI 只供应用管理运行时使用。如果你还要在终端中运行 `orchestra issue` 等命令，按[快速上手第 2 步](/cloud-quickstart#2-连接一台电脑)单独安装 CLI。
</Callout>
