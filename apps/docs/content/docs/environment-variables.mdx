...
description: Common server, storage, integration, and runtime configuration for self-hosting Orchestra.
---

import { Callout } from "fumadocs-ui/components/callout";

Orchestra reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/orchestra?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://orchestra.example.com
ORCHESTRA_APP_URL=https://orchestra.example.com
ORCHESTRA_PUBLIC_URL=https://api.orchestra.example.com
```
...
Do not use the default `JWT_SECRET` in production, and do not set `ORCHESTRA_DEV_VERIFICATION_CODE`.
</Callout>
...
|---|---|---|
| `DATABASE_URL` | local `orchestra` database | PostgreSQL connection string |
...
| `ORCHESTRA_SHUTDOWN_HOLD_DURATION` | `0` | How long to wait after a termination signal before graceful shutdown begins |
...
| `ORCHESTRA_APP_URL` | falls back to `FRONTEND_ORIGIN` | User-reachable web URL; CLI sign-in and account-link URLs use it |
| `ORCHESTRA_PUBLIC_URL` | empty | Public API URL; used for webhook URLs and runtime connection instructions |
...
| `RESEND_FROM_EMAIL` | `noreply@orchestra.local` | Sender address; must belong to a verified domain |
...
| `ORCHESTRA_DEV_VERIFICATION_CODE` | empty | Fixed 6-digit test code for non-production environments |

For the exact allowlist evaluation order, see [Sign-in and signup](/auth-setup#signup-restrictions).

## Attachment storage

When `S3_BUCKET` is unset, Orchestra uses local disk.
...
| `ORCHESTRA_TRUSTED_PROXIES` | empty | Trusted proxy CIDRs for automation webhooks and realtime connections |
...
| Lark | `ORCHESTRA_LARK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key |
| Slack | `ORCHESTRA_SLACK_SECRET_KEY` | Base64-encoded 32-byte token encryption key |
| Composio | `COMPOSIO_API_KEY` | Enables Composio tool connections |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | Callback API URL; can fall back to `ORCHESTRA_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state signing secret; can be derived from `JWT_SECRET` |
| Self-hosted Git | `ORCHESTRA_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab integration switch; enabled by default in compose |
| Self-hosted Git | `ORCHESTRA_VCS_SECRET_KEY` | Base64-encoded 32-byte encryption key (`openssl rand -base64 32`); the feature is entirely unavailable without it |
...
| Variable | Default | Description |
|---|---|---|
| `ORCHESTRA_LLM_API_KEY` | empty | OpenAI-compatible API key |
| `ORCHESTRA_LLM_BASE_URL` | empty | OpenAI-compatible endpoint |
| `ORCHESTRA_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | Used when a request does not specify a model |
...
| Variable | Default | Description |
|---|---|---|
| `ORCHESTRA_SERVER_URL` | `ws://localhost:8080/ws` | Orchestra API / WebSocket URL; also accepts `http(s)` |
| `ORCHESTRA_DAEMON_DEVICE_NAME` | hostname | Device name shown in the runtime list |
| `ORCHESTRA_AGENT_RUNTIME_NAME` | `Local Agent` | Runtime display name |
| `ORCHESTRA_DAEMON_POLL_INTERVAL` | `30s` | Task polling interval when no wake event arrives |
| `ORCHESTRA_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `ORCHESTRA_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Concurrent task ceiling per daemon |
| `ORCHESTRA_AGENT_TIMEOUT` | `0` | Absolute time limit per run; `0` means no limit |
| `ORCHESTRA_AGENT_IDLE_WATCHDOG` | `30m` | Silence ceiling with no output and no tool execution |
| `ORCHESTRA_AGENT_TOOL_WATCHDOG` | `2h` | Silence ceiling for a single tool call |
| `ORCHESTRA_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `ORCHESTRA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex semantic-silence threshold |
| `ORCHESTRA_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server startup handshake ceiling |
| `ORCHESTRA_DAEMON_AUTO_UPDATE` | Cloud `true`; self-hosted `false` | Whether to check for and apply CLI updates automatically |
| `ORCHESTRA_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | Update check interval |
| `ORCHESTRA_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `orchestra` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `ORCHESTRA_DAEMON_AUTO_UPDATE` |
| `ORCHESTRA_WORKSPACES_ROOT` | `~/orchestra_workspaces` | Root directory for task working directories |
| `ORCHESTRA_KEEP_ENV_AFTER_TASK` | `false` | Keep task directories for debugging |

Each AI coding tool accepts `ORCHESTRA_<PROVIDER>_PATH` and `ORCHESTRA_<PROVIDER>_MODEL` to override the command path and default model. Machine-wide default arguments via `ORCHESTRA_<PROVIDER>_ARGS` are currently supported for four tools: Claude Code, Codex, CodeBuddy, and Qwen Code. The variables are `ORCHESTRA_CLAUDE_ARGS`, `ORCHESTRA_CODEX_ARGS`, `ORCHESTRA_CODEBUDDY_ARGS`, and `ORCHESTRA_QWEN_ARGS`. For example:

```dotenv
ORCHESTRA_CLAUDE_PATH=/opt/bin/claude
ORCHESTRA_CLAUDE_ARGS=--max-turns 40
```

Precedence is command-line flag → environment variable → `~/.orchestra/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.orchestra/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.orchestra/profiles/<name>/config.json`:

```bash
orchestra config set poll_interval 10s
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Orchestra API / WebSocket URL |
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `orchestra config set poll_interval ""`.
