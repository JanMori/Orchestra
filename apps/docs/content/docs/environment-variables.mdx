...
ORCHESTRA_APP_URL=https://multica.example.com
ORCHESTRA_PUBLIC_URL=https://api.multica.example.com
```
...
| Variable | Default | Description |
|---|---|---|
| `ORCHESTRA_SERVER_URL` | `ws://localhost:8080/ws` | Multica API / WebSocket URL; also accepts `http(s)` |
...
| `ORCHESTRA_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `multica` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `ORCHESTRA_DAEMON_AUTO_UPDATE` |
| `ORCHESTRA_WORKSPACES_ROOT` | `~/multica_workspaces` | Root directory for task working directories |
