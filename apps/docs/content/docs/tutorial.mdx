...
description: "Start from an empty workspace and walk a personal website project through all of Orchestra's core flows: creating agents, delivering with issues, forming a crew, building skills, and setting up automation."
---

Orchestra is a platform where people and AI agents work together. This tutorial starts from nothing and covers:
...
The example project is a personal website, used to walk through Orchestra's overall flow and core features end to end.

**Before you start:**

- A Orchestra account (sign up at [localhost:5001](http://localhost:5001))
...
- **URL**: the slug in the workspace address — the `my-team` in `localhost:5001/my-team`. It cannot be changed after creation.
...
This tutorial uses [Orchestra Desktop](/docs): after downloading and signing in, it registers this computer as a runtime automatically and detects the installed AI coding tools. Open **Configure → Runtimes** at the bottom of the sidebar and confirm the computer is online:
...
Leave the remaining fields at their defaults and create. Back on the **Agents** page, Orchestra helper is online and can take work:

![Agent detail: Orchestra helper online](/images/docs/tutorial-agent-detail.png)
...
Open **Chat** in the sidebar, select Orchestra helper, and tell it to create an agent named Engineer on the same runtime, with Opus as the model, responsible for building and maintaining the personal website. Orchestra helper performs the creation on that computer and replies with the new agent's name and model. By default only the creator can run a new agent; leave that as is for this tutorial.

![Asking the helper in chat to create Engineer](/images/docs/tutorial-chat-create-engineer.png)

Open **Agents → Engineer** to check the result: the model is Opus, and the instructions were generated by Orchestra helper — scope of responsibility, how to get repository code, and requirements for submitting changes. They can be edited at any time.
...
That completes the setup. From here on, this is how day-to-day work runs in Orchestra: requirements are written as issues and handed to agents.
...
After submitting, Orchestra creates the issue (`MUL-1`, with a generated title) and sets Engineer as the assignee. Engineer starts executing and the status moves to In Progress. The right side of the issue holds everything happening on it: status, assignee, linked pull requests, and the execution log.
...
The MUL-1 loop works, but every hand-off started with you: mention Reviewer after reviewing, mention Engineer after the review. Coordination itself became your job. That layer can be handed off too. Orchestra's answer to it is the **crew**: a named group of agents and members led by a leader agent. When an issue is assigned to a crew, the leader takes it: reads the issue, delegates the work, and moves the status along. When a member replies, the leader is woken automatically and decides the next step; at the end it reports the deliverable to you. The leader takes over exactly the role you just played.

A crew is the division of labor you already have, plus a leader:
...
Describing this setup to Orchestra helper is enough: create the crew and its leader, give each member its job, attach one skill each to Lead and Engineer (covered in the next section), and put the workflow in the crew instructions — development goes to Engineer, delivery waits until the PR passes review, and Engineer merges after your sign-off. The helper applies the whole configuration in one pass with the CLI:

![The helper configuring the whole crew in one pass](/images/docs/tutorial-helper-create-crew.png)

Open **Crews → Website Team** in the sidebar to check the result: the leader, the members, and the role descriptions are all there. Crews can also be created from a form on this page; the full mechanism is in [Crews](/crews):

![Crew detail: leader, members, and roles](/images/docs/tutorial-crew-detail.png)
...
A skill is a knowledge package for an agent: one `SKILL.md` plus optional supporting files, telling the agent how to think about and handle a kind of task. Orchestra follows the Anthropic Agent Skills open standard, so any skill that matches the spec can be imported directly; once attached to an agent, it syncs to the runtime at execution time. Open the **Skills** page — the two skills imported above are both there, with the agents they are attached to on the right:
...
Read the files of a third-party skill before importing it: Orchestra does not sandbox them, and a skill's content is handed to the AI coding tool as-is.

## The second issue: hand it to the crew

Give the crew a complete piece of work: turn the site into a generic template — replace the real name with a placeholder photographer name and the real email with `hello@example.com`, leaving layout and content structure untouched. This issue does not have to be written by hand: describe the requirement to Lead in Chat and create it with **Create with agent**, assigned to the crew — a crew, like an agent or a member, can appear anywhere an assignee can:
...
![MUL-2: Lead creates the issue and starts work](/images/docs/tutorial-issue-crew-created.png)
...
![Lead delegates and Engineer reports the PR](/images/docs/tutorial-crew-dispatch.png)
...
![Lead's delivery report](/images/docs/tutorial-crew-delivery.png)
...
Configuring it takes one sentence: tell Orchestra helper that at nine every Monday morning, Reviewer should run a full check over the site's repository — redundancy and dead code, questionable structure, places that could follow conventions better — writing findings into issue comments and changing no code:
...
Triggers support webhooks in addition to schedules, and the executor can also point at a crew — see [Autopilots](/autopilots).

## Working with other people

So far you have been the only human member of the workspace. Under **Settings → Members**, invite others by email; they create issues, @-mention agents, and join crews the same way you do — several people and several agents working together on the same set of issues:
...
Agents are first-class members and all collaboration stays on the issue timeline — that is the way of working Orchestra provides. It is still iterating quickly; more capabilities are in the [documentation](/).

## Next steps

- [Core concepts](/concepts) — meet all of Orchestra's core objects in three minutes.
- [Crews](/crews) — how a leader routes work, and when to use a crew instead of a single agent.
