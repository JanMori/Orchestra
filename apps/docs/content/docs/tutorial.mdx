...
description: "Start from an empty workspace and walk a personal website project through all of Multica's core flows: creating agents, delivering with issues, forming a crew, building skills, and setting up automation."
---
...
The MUL-1 loop works, but every hand-off started with you: mention Reviewer after reviewing, mention Engineer after the review. Coordination itself became your job. That layer can be handed off too. Multica's answer to it is the **crew**: a named group of agents and members led by a leader agent. When an issue is assigned to a crew, the leader takes it: reads the issue, delegates the work, and moves the status along. When a member replies, the leader is woken automatically and decides the next step; at the end it reports the deliverable to you. The leader takes over exactly the role you just played.
...
Describing this setup to Multica helper is enough: create the crew and its leader, give each member its job, attach one skill each to Lead and Engineer (covered in the next section), and put the workflow in the crew instructions — development goes to Engineer, delivery waits until the PR passes review, and Engineer merges after your sign-off. The helper applies the whole configuration in one pass with the CLI:
