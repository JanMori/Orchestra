...
<Callout type="warn">
**Self-hosted Orchestra only.** This integration is available only when you run
Orchestra yourself; it is not offered on Orchestra Self-Host. "Self-hosted" here means
**Orchestra** is self-hosted — typically so it can reach a Git instance on your own
network. A server operator must enable it (`ORCHESTRA_VCS_INTEGRATION_ENABLED=true`)
and set `ORCHESTRA_VCS_SECRET_KEY`; until then the section does not appear in
Settings → Integrations.
</Callout>

Orchestra connects to a self-hosted Git provider per workspace: **Forgejo**, **Gitea**, or **GitLab**. Once connected, any pull request (GitLab: merge request) whose branch name, title, or body contains an issue identifier (for example `TASK-123`) is **auto-linked** to that [issue](/issues), appears under **Pull requests** in the issue sidebar, and — when merged with a closing keyword — moves the issue to **Done**. CI for the head commit shows as a checks bar on the card.
...
```sh
ORCHESTRA_VCS_INTEGRATION_ENABLED=true
```
...
```sh
ORCHESTRA_VCS_SECRET_KEY=<base64 32-byte key>
```
...
Set `ORCHESTRA_PUBLIC_URL` to the server's public base URL so Orchestra can show a ready-to-paste webhook URL. Without it, the UI shows only the webhook path and you prepend your own origin.
...
2. In Orchestra, open **Settings → Integrations → Git providers**.
3. Choose the **provider**, enter the instance URL (for example `https://forgejo.example.com`) and the access token, then **Connect**. Orchestra validates the token against the instance before saving.
...
Orchestra authenticates every delivery against the stored secret, so a webhook without the matching secret is rejected.

## What gets mirrored

- **Pull / merge requests** — open, closed, merged, and draft state, author, branch, and (where the provider supplies them) diff stats.
- **Issue links** — identifiers in the title, body, or branch link the PR to the issue. A closing keyword (`Closes`/`Fixes`/`Resolves TASK-123`) on a merged PR moves the issue to **Done** once no linked PR is still open.
