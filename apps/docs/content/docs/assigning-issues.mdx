...
When the assignee is a [crew](/crews), the same expectations apply: Multica enqueues the leader, who moves the parent issue to `in_progress` on the first turn and keeps it there while members work. The leader only moves it to `in_review` once the overall goal is met — a successful first dispatch is not completion.
