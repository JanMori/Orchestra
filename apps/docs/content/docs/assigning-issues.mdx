...
When you assign to a crew, the crew becomes the issue's assignee and the crew leader starts first. It decides whether to hand work to other members, following how the crew operates.

## Execution after assignment

Orchestra creates a [task](/tasks) for the agent. Once an online runtime claims the task, it invokes the AI coding tool on that machine; progress, comments, and results then appear in the issue.
...
When the assignee is a [crew](/crews), the same expectations apply: Orchestra enqueues the leader, who moves the parent issue to `in_progress` on the first turn and keeps it there while members work. The leader only moves it to `in_review` once the overall goal is met — a successful first dispatch is not completion.
...
```bash
orchestra issue assign MUL-42 --to "Agent name"
orchestra issue assign MUL-42 --unassign
```

In scripts, use `--to-id <uuid>` to avoid matching the wrong object with the same name. Look up UUIDs with `orchestra agent list --output json` or `orchestra workspace member list --output json`.
...
- [Crews](/crews) — have a leader coordinate multiple agents.
