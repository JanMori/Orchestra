...
description: How Orchestra connects computers, discovers AI coding tools, and executes tasks.
---

import { Callout } from "fumadocs-ui/components/callout";

Orchestra records and coordinates the work; connected computers execute it. The **daemon** on a computer claims tasks and invokes the AI coding tools installed on that machine.

## Daemon vs runtime

- The **daemon** is the Orchestra background process running on one computer. It connects to the server, discovers local tools, claims tasks, and reports results back.
...
The AI coding tools a local runtime invokes, those tools' own login credentials, and your local code directories all stay on the connected computer. The Orchestra server does not execute commands on behalf of local tools, and it does not automatically upload your entire working directory.
...
With Orchestra Desktop, the app starts the daemon automatically — no extra command needed.

On the web, a remote computer, or a headless environment, [install the Orchestra CLI](/cli) first, then run:
...
| Command | Purpose |
| --- | --- |
| `orchestra daemon status` | Show daemon and connection status |
| `orchestra daemon logs -f` | Follow the logs |
| `orchestra daemon restart` | Restart the daemon and re-detect local tools |
| `orchestra daemon stop` | Stop the daemon |
| `orchestra daemon start --foreground` | Run in the current terminal, for debugging |
...
Once a limit is reached, new tasks keep queueing. You can adjust a single agent's concurrency in the agent settings, and the machine-wide cap through `ORCHESTRA_DAEMON_MAX_CONCURRENT_TASKS`. Parallel runs compete for machine capacity, tool account quota, and the same working directory all at once.
...
A custom profile does not add a new communication protocol. You still pick one of the protocol families Orchestra already supports (the tool's integration protocol type; see [AI coding tools comparison](/providers)), and the command itself must be compatible with that family.
...
```bash
orchestra runtime profile set-path <profile-id> --path /absolute/path/to/command
```

Remove the path override:

```bash
orchestra runtime profile unset-path <profile-id>
```
...
1. Run `orchestra daemon status` to confirm the daemon is running.
2. Run `orchestra daemon logs -f` to look for login, network, or tool-detection errors.
3. Run `command -v <tool-command>` in the same environment to confirm the daemon can find it.
4. Open Orchestra's **Runtimes** page and check whether the target computer and the matching AI coding tool show as online.
5. After a tool install, a path change, or a profile update, run `orchestra daemon restart`.
