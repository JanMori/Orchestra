...
| 변수 | 기본값 | 설명 |
|---|---|---|
| `ORCHESTRA_SERVER_URL` | `ws://localhost:8080/ws` | Multica API / WebSocket 주소. `http(s)`도 허용 |
...
| `ORCHESTRA_DAEMON_AUTO_RELOAD` | `true` | 외부에서 교체된 `multica` 바이너리(`brew upgrade`, 재다운로드, 로컬 빌드)로 재시작할지 여부. `ORCHESTRA_DAEMON_AUTO_UPDATE`와 독립적 |
| `ORCHESTRA_WORKSPACES_ROOT` | `~/multica_workspaces` | 실행 태스크 작업 디렉터리의 루트 |
