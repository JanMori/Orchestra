...
インストール後、Web 版と同じメールアドレスでサインインします。サインインすると、Desktop は内蔵の Orchestra CLI を起動し、マシンにすでにインストールされている AI コーディングツールを検出します。

<Callout type="info">
Desktop に内蔵された CLI は、アプリが管理するランタイム専用です。ターミナルで `orchestra issue` などのコマンドも実行したい場合は、[クイックスタートのステップ 2](/cloud-quickstart#2-コンピュータを接続する)に従って CLI を別途インストールしてください。
</Callout>
