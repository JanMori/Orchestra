---
title: Using the CLI
description: Install and log in to the Orchestra CLI, and manage workspaces, issues, agents, and runtimes from the terminal.
---

import { Callout } from "fumadocs-ui/components/callout";

The Orchestra CLI both connects local runtimes and manages workspaces, issues, agents, and automations from the terminal. This page covers the common paths; the flags your installed version supports are always defined by each command's own `--help`.
...
Connect to Orchestra Self-Host:
...
If you only need to sign in again without overwriting other configuration, run `orchestra login`. On a machine without a browser, create a personal access token in the web settings first, then enter it with:

```bash
orchestra login --token
```
...
Subsequent commands use this workspace. A single command can override it with `--workspace-id`, or you can set `ORCHESTRA_WORKSPACE_ID`.

Invite members:

```bash
orchestra workspace member invite teammate@example.com
orchestra workspace member invite admin@example.com --role admin
```
...
orchestra issue get TASK-123
orchestra issue search "login failure"

# Create and update
orchestra issue create --title "Fix login failure"
orchestra issue status TASK-123 in_progress
orchestra issue assign TASK-123 --to "Backend Agent"

# Comments and runs
orchestra issue comment list TASK-123
orchestra issue comment add TASK-123 --content "Check the regression tests first"
orchestra issue runs TASK-123
```

Read long descriptions or comments from stdin to avoid dealing with newlines and quoting:

```bash
orchestra issue create --title "Upgrade notes" --description-stdin < notes.md
orchestra issue comment add TASK-123 --content-stdin < review.md
```

View the messages of a single run, or stop a task:

```bash
orchestra issue run-messages <task-id> --issue TASK-123
orchestra issue cancel-task <task-id> --issue TASK-123
```
...
orchestra agent get <agent-id>
orchestra agent create --help
orchestra agent update <agent-id> --help

orchestra skill list
orchestra skill get <skill-id>
orchestra skill import --url <skill-url>
orchestra agent skills add <agent-id> --skill-ids <skill-id>
```

When an import hits a skill with the same name, the default is to stop without modifying existing content. Pick by intent:

```bash
orchestra skill import --url <skill-url> --on-conflict overwrite
orchestra skill import --url <skill-url> --on-conflict rename
orchestra skill import --url <skill-url> --on-conflict skip
```
...
orchestra daemon logs --follow

orchestra runtime list
orchestra runtime rename <runtime-id> "Office Mac"
orchestra runtime usage <runtime-id>
orchestra runtime activity <runtime-id>
```

Deleting a runtime that still has active agents bound to it is refused by default. `orchestra runtime delete <id> --cascade` unbinds those agents, preserves their configuration and history, and cancels their active tasks.
...
| `agent`, `skill`, `crew` | Manage agents, skills, and crews |
...
`orchestra chat` reads **the external chat session an agent is currently handling**; it is mainly for agents in chat integrations, not a general command for browsing arbitrary workspace conversations.

## IDs and output formats

Issues use keys like `TASK-123` or full UUIDs; short UUID prefixes are not accepted.
...
Scripts should use JSON output rather than parsing the terminal-oriented tables. The output formats and pagination flags each command supports are defined by `orchestra <command> --help`.

## Profiles and configuration

The default configuration lives in `~/.orchestra/config.json`. Use `--profile <name>` to isolate a separate set of server address, token, default workspace, and daemon state:

```bash
orchestra setup self-host --profile staging \
  --server-url https://api.staging.example.com \
  --app-url https://app.staging.example.com

orchestra issue list --profile staging
```

Named profiles live in `~/.orchestra/profiles/<name>/config.json`. Inspect the current values:

```bash
orchestra config show
orchestra config show --profile staging
```

<Callout type="warning">
CLI configuration files contain tokens that can access Orchestra as you. Don't commit them to a repository, upload them to logs, or share them with others.
</Callout>

## Command reference

The tables below cover every current top-level command, grouped the way the CLI itself groups them. All commands accept the global flags `--server-url`, `--workspace-id`, `--profile`, and `--debug`. `--output` is registered per command, and the default format varies: `list` commands default to a table, while `get` and `create` commands mostly default to JSON. Full flags are defined by `orchestra <command> --help`.
...
| | `assign <id>` | Assign or unassign | `--to` (fuzzy name match against members, agents, and crews), `--to-id`, `--unassign` |
...
| `crew` | `list/get/create/update/delete` | Manage crews (`delete` archives) | |
| | `member list/add/set-role/remove` | Manage crew members | |
...
| Command | Subcommand | Purpose | Key flags |
| --- | --- | --- | --- |
| `daemon` | `start` | Start the local daemon | `--foreground`, `--device-name`, `--runtime-name`, `--poll-interval`, `--heartbeat-interval`, `--agent-timeout` (`0` means no limit), `--max-concurrent-tasks`, `--no-auto-update`; each has a matching `ORCHESTRA_*` environment variable |
