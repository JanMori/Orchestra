...
orchestra issue get MUL-123
orchestra issue search "login failure"

# Create and update
orchestra issue create --title "Fix login failure"
orchestra issue status MUL-123 in_progress
orchestra issue assign MUL-123 --to "Backend Agent"

# Comments and runs
orchestra issue comment list MUL-123
orchestra issue comment add MUL-123 --content "Check the regression tests first"
orchestra issue runs MUL-123
```
...
orchestra issue comment add MUL-123 --content-stdin < review.md
```

View the messages of a single run, or stop a task:

```bash
orchestra issue run-messages <task-id> --issue MUL-123
orchestra issue cancel-task <task-id> --issue MUL-123
```
