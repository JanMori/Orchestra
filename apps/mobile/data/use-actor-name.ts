...
/**
 * Resolve actor (member / agent / crew) name + avatar URL from the
...
export function useActorLookup() {
  ...
  const getName = (
    type: "member" | "agent" | "crew" | null | undefined,
    ...
    return crews.find((s) => s.id === id)?.name ?? "Crew";
  };
  ...
  const getAvatarUrl = (
    ...
    return crews.find((s) => s.id === id)?.avatar_url ?? null;
  };
