 ...
 * members + agents + crews, plus an "Unassigned" option. See
...
function isRowSelected(value: AssigneeValue, row: Row): boolean {
  ...
  return value.type === "crew" && value.id === row.crew.id;
}
...
export function AssigneePickerBody({ value, query, onChange }: Props) {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    const crewRows: Row[] = [...crews]
      .filter((s) => !s.archived_at && matchName(s.name))
      .sort((a, b) => a.name.localeCompare(b.name))
      .map((s) => ({ kind: "crew" as const, crew: s }));

    if (q) return [...memberRows, ...agentRows, ...crewRows];
    ...
    const all = [...memberRows, ...agentRows, ...crewRows];
    ...
    return [
      ...
      ...crewRows.filter((r) => !isRowSelected(value, r)),
    ];
  }, [members, agents, crews, query, value]);
  ...
  const select = (row: Row) => {
    ...
    else onChange({ type: "crew", id: row.crew.id });
  };
