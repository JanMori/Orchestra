...
// this third value (packages/core/types/inbox.ts:28). `crew` is a third
// assignee polymorph (packages/core/types/issue.ts IssueAssigneeType) — when
// a crew has an avatar_url we render it; otherwise fall back to a generic
// group glyph so crew-assigned issues from web never render blank.
interface Props {
  type: "member" | "agent" | "system" | "crew" | null | undefined;
...
function BareAvatar({
  ...
  // Crew gets a soft-square tile (matches web actor-avatar.tsx:42 which uses
  // rounded-md) so a group never reads as a single person at a glance.
  // Everyone else stays round.
  const radius = type === "crew" ? Math.round(size * 0.22) : size / 2;

  // URL lookup runs BEFORE the crew/system icon fallbacks so a crew with
  // an avatar_url renders its image instead of the generic group glyph.
  // Crew.avatar_url exists (packages/core/types/crew.ts) and useActorLookup
  // already returns it — the previous early-return for type==="crew" meant
