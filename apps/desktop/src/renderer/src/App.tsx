...
import { pickLocale, type SupportedLocale } from "@orchestra/core/i18n";
import { useAuthStore } from "@orchestra/core/auth";
import { useWelcomeStore } from "@orchestra/core/onboarding";
import { workspaceKeys, workspaceListOptions } from "@orchestra/core/workspace/queries";
import { api } from "@orchestra/core/api";
import { useHasOnboarded } from "@orchestra/core/paths";
import { setCurrentWorkspace } from "@orchestra/core/platform";
import { ThemeProvider } from "@orchestra/ui/components/common/theme-provider";
import { MulticaIcon } from "@orchestra/ui/components/common/multica-icon";
import { Toaster } from "@orchestra/ui/components/ui/sonner";
...
import { captureEvent } from "@orchestra/core/analytics";
import { RESOURCES } from "@orchestra/views/locales";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.orchestra config).
  useEffect(() => {
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-body text-muted-foreground">
          Orchestra Desktop could not load <code>~/.multica/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
