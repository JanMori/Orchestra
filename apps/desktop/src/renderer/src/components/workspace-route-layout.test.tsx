...
vi.mock("@orchestra/core/workspace", async () => {
  const actual = await vi.importActual<typeof import("@orchestra/core/workspace")>(
    "@orchestra/core/workspace",
  );
  return {
    ...actual,
...
vi.mock("@orchestra/views/workspace/use-workspace-seen", () => ({
  useWorkspaceSeen: () => state.workspaceSeen,
}));

vi.mock("@orchestra/views/workspace/welcome-after-onboarding", () => ({
  WelcomeAfterOnboarding: () => null,
}));

vi.mock("@orchestra/views/layout", () => ({
...
vi.mock("@orchestra/views/onboarding", () => ({
  SourceBackfillModal: () => {
