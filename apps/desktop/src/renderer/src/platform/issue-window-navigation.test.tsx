/**
 * ISS-5208 — a dedicated issue window must answer content-link navigation.
 *
...
function renderWindow() {
  return render(
    <MemoryRouter initialEntries={["/acme/issues/ISS-1"]}>
      <Routes>
        <Route
...
describe("IssueWindowNavigationProvider content links", () => {
  it("opens another issue in place", () => {
    ...
    navigate("/acme/issues/ISS-2");

    expect(screen.getByTestId("path")).toHaveTextContent("/acme/issues/ISS-2");
  ...
  it("hands a page this window cannot host to the browser instead of swallowing it", () => {
    ...
    expect(screen.getByTestId("path")).toHaveTextContent("/acme/issues/ISS-1");
