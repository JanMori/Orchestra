...
// ISS-4741: the adapter mutates tab sessions in the REAL store (no router,
...
describe("routeContentLinkPath (links inside content — ISS-5208)", () => {
  it("opens a same-workspace path in a foreground tab of its own", () => {
    routeContentLinkPath("/acme/issues/ISS-1");

    const group = useTabStore.getState().byWorkspace.acme;
    const opened = group.tabs.find((t) => t.url === "/acme/issues/ISS-1")!;
