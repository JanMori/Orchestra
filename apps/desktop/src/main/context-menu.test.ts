...
describe("installContextMenu — link items", () => {
  ...
  it("adds 'Open Link in Browser' and 'Copy Link Address' when right-clicking an http(s) link", () => {
    ...
    // before ISS-3083 follow-up, right-clicking an <a> in the
    ...
    wc.fire({
      ...baseSelection({ linkURL: "http://localhost:5001/welcome" }),
    });
    ...
    expect(ctx.openExternalSpy).toHaveBeenCalledWith("http://localhost:5001/welcome");
  ...
  it("uses zh-Hans labels when the OS preferred language is Chinese", () => {
    ...
    const wc = makeWebContents();
    installContextMenu(wc as never);
    wc.fire(baseSelection({ linkURL: "http://localhost:5001" }));
