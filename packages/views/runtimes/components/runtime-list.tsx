...
function RuntimeNameCell({
   ...
   * (ISS-5248). Omitted when the row has no machine context (orphan custom
...
export function CliCell({ runtime }: { runtime: AgentRuntime }) {
  ...
  // The separate `cli_version` is the shared orchestra daemon CLI, identical
...
export function RuntimeRowMenu({
  ...
  // re-registers within seconds), but ISS-3352 surfaced that owners read
...
export function RuntimeList({
   ...
   * while a per-runtime alias still shows (ISS-5248).
   */
  machineTitle?: string;
}) {
  const { t } = useT("runtimes");
