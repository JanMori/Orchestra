...
import type { AgentTask, Issue } from "@orchestra/core/types";
...
// "3 agents working" for those two agents, contradicting the chip. ISS-3872.
...
vi.mock("@orchestra/core/runtimes/queries", () => ({
...
vi.mock("@orchestra/ui/components/common/actor-avatar", () => ({
...
describe("WorkspaceAgentActivityHoverContent", () => {
  it("carries the two figures the chip does not, and groups rows by issue", () => {
    renderWithI18n(
      <WorkspaceAgentActivityHoverContent
        issues={[
          makeIssue("i-1", "ISS-4879", "Counting logic looks wrong"),
          makeIssue("i-2", "ISS-4881", "daemon extra work dir"),
          makeIssue("i-3", "ISS-4883", "First PR review flow"),
        ]}
        tasksByIssueId={
          new Map([
            [
    ...
    expect(screen.getByText("ISS-4879")).toBeInTheDocument();
  ...
  it("says nothing about work it excludes", () => {
    ...
    renderWithI18n(
      <WorkspaceAgentActivityHoverContent
        issues={[makeIssue("i-1", "ISS-1", "One")]}
        tasksByIssueId={
