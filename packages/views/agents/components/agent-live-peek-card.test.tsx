...
vi.mock("@orchestra/core/agents", async () => {
  const actual =
    await vi.importActual<typeof import("@orchestra/core/agents")>(
      "@orchestra/core/agents",
    );
  return {
    ...actual,
...
describe("AgentLivePeekCard", () => {
  it("renders Working state with the linked current issue", () => {
    ...
    const link = screen.getByRole("link", { name: /ISS-42/ });
