...
} from "@orchestra/core/inbox/queries";
 ...
 * core is `@orchestra/core/paths` (`parseTabSubject` + `resolveTabPresentation`).
 *
...
function useTabEntityData(subject: TabSubject, wsId: string): TabEntityData {
  ...
  const issueId =
    subject.kind === "issue"
      ? subject.id
      : (inboxItem?.issue_id ?? "");
  // An issue tab's URL segment may be a human-readable identifier (`TASK-123`).
