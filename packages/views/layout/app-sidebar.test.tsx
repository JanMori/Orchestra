...
vi.mock("@orchestra/ui/components/ui/sidebar", () => ({
...
vi.mock("@orchestra/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@orchestra/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));
...
vi.mock("@orchestra/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@orchestra/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@orchestra/core/inbox/queries", () => ({
...
vi.mock("@orchestra/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@orchestra/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@orchestra/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@orchestra/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@orchestra/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@orchestra/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@orchestra/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
...
describe("PinRow", () => {
  ...
  it("renders loaded details", async () => {
    detail.current = { isPending: false, isError: false, data: { identifier: "TASK-123", title: "Keep this pin", status: "todo" }, error: null };
    render(<AppSidebar />);
    expect(await screen.findByText("Keep this pin")).toBeInTheDocument();
    expect(screen.queryByText("TASK-123 Keep this pin")).not.toBeInTheDocument();
  });
  ...
  it("does not also highlight the parent workspace nav for an active pin", async () => {
    ...
    detail.current = {
      isPending: false,
      isError: false,
      data: { identifier: "TASK-123", title: "Keep this pin", status: "todo" },
      error: null,
    };
