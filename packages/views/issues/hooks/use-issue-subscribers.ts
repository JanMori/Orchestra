...
import type { IssueSubscriber } from "@orchestra/core/types";
import type {
  SubscriberAddedPayload,
  SubscriberRemovedPayload,
} from "@orchestra/core/types";
import { issueSubscribersOptions, issueKeys } from "@orchestra/core/issues/queries";
import {
  useToggleIssueSubscriber,
  useUnsubscribeFromIssueSubtree,
} from "@orchestra/core/issues/mutations";
import { useWSEvent, useWSReconnect } from "@orchestra/core/realtime";
// Imported from the defining module, not the "@orchestra/core/api" barrel: the
...
import { ApiError } from "@orchestra/core/api/client";
...
export function useIssueSubscribers(issueId: string, userId?: string) {
      ...
      (payload: unknown) => {
        ...
        qc.setQueryData<IssueSubscriber[]>(
          ...
          (old) => {
            ...
            // delegation that no longer applies (ISS-5483).
            if (existing) {
  ...
  // unknown as well; only a resolved one is truth (ISS-5714).
  ...
  // reads as the product being creepy unless it says why (ISS-5483).
  ...
  // rolls back to it instead of to the server's state (ISS-5714).
  ...
  // a button that never fired. Say so instead (ISS-5714).
  ...
  // flipping to confirm the click landed (ISS-5714).
