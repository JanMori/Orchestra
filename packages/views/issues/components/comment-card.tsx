...
import { Card } from "@orchestra/ui/components/ui/card";
...
import type { TimelineEntry, Attachment } from "@orchestra/core/types";
import { contentReferencesAttachment } from "@orchestra/core/types";
import { selectStandaloneAttachments } from "@orchestra/core/attachments/image-sequence";
import { useCommentCollapseStore, useCommentDraftStore } from "@orchestra/core/issues/stores";
...
export function AttachmentList({
  ...
  // Shared with the image-sequence builder (ISS-5752) so the cards rendered
...
function TaskCommentRetryButton({
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      // Rerun re-checks the operator's invoke permission (ISS-4525); a
...
function useEditAttachmentState(
  ...
  // Uploads for this edit session (ISS-5181) — coordinator-owned, persisted in
  ...
  // Await-then-render save (ISS-5181): shared submit contract, with the edit-
  ...
  // Stale-submit guard (ISS-5181 P0) — see CommentInput. The edit hook lives
  ...
  const { submitting: saving, submit: saveEdit } = useComposerSubmit({
    ...
    onSubmit: async (trimmed) => {
      ...
      const activeIds = collectActiveAttachmentIds(
        trimmed,
        [...(entry.attachments ?? []), ...pendingAttachments],
        // Body-referenced + retained-standalone only (ISS-5181): an upload the
...
// A quick action posts an ordinary comment and is rendered as one (ISS-5465).
