...
import type { CommentTriggerPreviewAgent, CommentTriggerOutcome } from "@orchestra/core/types";
import { useAgentPresenceDetail } from "@orchestra/core/agents";
import { mentionLabelsByTarget } from "@orchestra/core/issues/comment-trigger-outcomes";
...
import { AVATAR_SIZE_PX } from "@orchestra/ui/lib/avatar-size";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
interface CommentTriggerChipsProps {
  ...
  // Explicit @agent / @crew mentions that will NOT trigger if posted as-is
  // (ISS-4525 §2). Each renders as a named warning chip so the user sees WHICH
...
function sourceLabel(source: string, t: IssuesT): string {
  switch (source) {
    ...
    case "mention_crew_leader":
      return t(($) => $.comment.trigger_source_mention_crew_leader);
    default:
...
// just restate it. Only the crew-leader link (non-obvious) and the unknown
...
function sourceReason(agent: CommentTriggerPreviewAgent, t: IssuesT): string | null {
  switch (agent.source) {
    ...
    case "mention_agent":
      return null;
    case "mention_crew_leader":
      return t(($) => $.comment.trigger_reason_mention_crew_leader);
    default:
