...
import { ResizablePanelGroup, ResizablePanel, ResizableHandle } from "@orchestra/ui/components/ui/resizable";
import { Sheet, SheetContent } from "@orchestra/ui/components/ui/sheet";
...
import { collectImageSequence, type ImageSequenceBlock } from "@orchestra/core/attachments/image-sequence";
...
import { Popover, PopoverTrigger, PopoverContent } from "@orchestra/ui/components/ui/popover";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@orchestra/ui/components/ui/dialog";
import { Checkbox } from "@orchestra/ui/components/ui/checkbox";
import { Command, CommandInput, CommandList, CommandEmpty, CommandGroup, CommandItem } from "@orchestra/ui/components/ui/command";
import { AvatarGroup, AvatarGroupCount } from "@orchestra/ui/components/ui/avatar";
import { ActorAvatar } from "../../common/actor-avatar";
import { PropRow } from "../../common/prop-row";
import { PropertyIcon } from "../../common/property-icon";
import type { Attachment, Issue, IssueProperty, IssueStatus, IssuePriority, TimelineEntry, UpdateIssueRequest } from "@orchestra/core/types";
import { contentReferencesAttachment } from "@orchestra/core/types";
import { STATUS_CONFIG, PRIORITY_CONFIG } from "@orchestra/core/issues/config";
import { formatDateOnly, isPastDateOnly } from "@orchestra/core/issues/date";
import { useUpdateIssue } from "@orchestra/core/issues/mutations";
...
import { useGitHubSettings } from "@orchestra/core/github";
...
import { useRecentContextStore } from "@orchestra/core/chat";
import { issueListOptions, issueDetailOptions, childIssuesOptions, childIssueProgressOptions, issueAttachmentsOptions } from "@orchestra/core/issues/queries";
import { projectDetailOptions } from "@orchestra/core/projects/queries";
import { ProjectIcon } from "../../projects/components/project-icon";
import { issueLabelsOptions } from "@orchestra/core/labels";
...
import { useIssueSelectionStore } from "@orchestra/core/issues/stores/selection-store";
import { BatchActionToolbar } from "./batch-action-toolbar";
import { useIssueTimeline } from "../hooks/use-issue-timeline";
import { useIssueReactions } from "../hooks/use-issue-reactions";
import { useIssueSubscribers } from "../hooks/use-issue-subscribers";
import { ReactionBar } from "@orchestra/ui/components/common/reaction-bar";
...
function SubscriberPopoverContent({
   ...
   * for a real answer, not just for the in-flight mutation (ISS-5714).
   */
...
function formatActivity(
  ...
  switch (entry.action) {
    ...
    case "crew_leader_evaluated": {
      const reason = details.reason?.trim();
      switch (details.outcome) {
        case "action":
          return reason
            ? t(($) => $.activity.crew_leader_action_reason, { reason })
            : t(($) => $.activity.crew_leader_action);
        case "no_action":
          return reason
            ? t(($) => $.activity.crew_leader_no_action_reason, { reason })
            : t(($) => $.activity.crew_leader_no_action);
        case "failed":
          return reason
            ? t(($) => $.activity.crew_leader_failed_reason, { reason })
            : t(($) => $.activity.crew_leader_failed);
        default:
          return t(($) => $.activity.crew_leader_evaluated);
      }
    }
    default:
      return entry.action ?? "";
...
export function IssueDetail({ issueId, onDelete, onDone, defaultSidebarOpen = true, layoutId = "multica_issue_detail_layout", highlightCommentId, leadingAction }: IssueDetailProps) {
  ...
  // Pull-based scroll restoration (ISS-4741): the platform serves the offset
  ...
  const timelineView = useMemo(() => {
    ...
    // - crew_leader_evaluated: never coalesce; outcome/reason are audit data
    const COALESCE_MS = 2 * 60 * 1000;
    const NO_TIME_LIMIT_ACTIONS = new Set(["task_completed", "task_failed"]);
    const NEVER_COALESCE_ACTIONS = new Set(["crew_leader_evaluated"]);
  ...
  // nested under it (ISS-5752). Built from `items` rather than the flat
  ...
  const detailContent = (
    ...
    <div className="relative flex h-full min-w-0 flex-1 flex-col">
        {/* In-page find bar — floats over the top-right of the content column
            ...
            its close button (ISS-4414). On desktop it also steps inside the
        ...
        {/* scrollbar-gutter both-edges: with classic (space-taking) scrollbars —
            ...
            centered column reads 32px left vs 43px right (ISS-4404). Mirroring
          ...
          <div {...descDropZoneProps} className="relative mt-5 rounded-lg">
            <ContentEditor
              ...
              onUpdate={(md) => {
                ...
                // bind via `contentReferencesAttachment` (ISS-3130 / ISS-3192).
              ...
              // the image markdown and its attachment_ids bind (ISS-3254).
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {/* A delegated subscription is one the user never opted into
                    ...
                    adding them to things (ISS-5483). */}
                {isSubscribed && subscriptionReason === "delegated" && (
                  <Tooltip>
                    {/* Quiet surface, not plain body text: this is metadata
                        ...
                        caption role rather than an ad-hoc size (ISS-5451). */}
        ...
        {/* Thread quick-jump rail — rides the scroll container's right edge,
            ...
            scrolling (ISS-4522). right-3 is the inset that works in both
