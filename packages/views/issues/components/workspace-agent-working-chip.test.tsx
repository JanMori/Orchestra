...
// chip's own wiring to the hover body is observable: the ISS-5525 follow-up bug
// was in that wiring (`agents ?? []`), not in the body's rendering.
vi.mock("@orchestra/ui/components/ui/hover-card", () => ({
...
vi.mock("@orchestra/ui/components/ui/button", async () => {
  const actual =
    await vi.importActual<typeof import("@orchestra/ui/components/ui/button")>(
      "@orchestra/ui/components/ui/button",
    );
  return {
    ...actual,
...
describe("WorkspaceAgentWorkingChip", () => {
  ...
  // The whole point of ISS-5525: the chip must not invent a count of its own.
