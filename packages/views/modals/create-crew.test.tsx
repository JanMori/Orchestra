...
vi.mock("@orchestra/core/workspace/queries", () => ({
  ...
  workspaceKeys: { crews: (id: string) => ["crews", id] },
}));

vi.mock("@orchestra/core/api", () => ({
  api: {
    createCrew: (...args: unknown[]) => mocks.createCrew(...args),
    addCrewMember: (...args: unknown[]) => mocks.addCrewMember(...args),
  },
}));
...
vi.mock("@orchestra/core/paths", () => ({
  useWorkspacePaths: () => ({
    crewDetail: (id: string) => `/test-ws/crews/${id}`,
  }),
}));

vi.mock("@orchestra/core/utils", () => ({
...
vi.mock("@orchestra/ui/components/ui/popover", () => ({
...
vi.mock("@orchestra/ui/components/ui/dialog", () => ({
...
vi.mock("@orchestra/ui/components/ui/button", () => ({
  Button: ({
    children,
    disabled,
    onClick,
...
vi.mock("@orchestra/ui/components/ui/input", () => ({
  Input: (props: React.InputHTMLAttributes<HTMLInputElement>) => (
    <input {...props} />
  ),
}));

vi.mock("@orchestra/ui/components/ui/label", () => ({
...
import { CreateCrewModal } from "./create-crew";
...
// "Create Crew" is both the dialog title and the submit button label. Always
// pick by role so tests don't depend on DOM order between the two.
function getSubmitButton(): HTMLButtonElement {
  const btn = screen
    .getAllByRole("button")
    .find((b) => b.textContent === "Create Crew");
  if (!btn) throw new Error("Create Crew submit button not found");
...
function renderModal() {
  ...
  render(
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <CreateCrewModal onClose={onClose} />
    </I18nProvider>,
  );
  return { onClose };
}
...
describe("CreateCrewModal", () => {
  ...
  it("binds the name and description inputs in the identity row", () => {
    ...
    const desc = screen.getByPlaceholderText(/Describe what this crew/i) as HTMLInputElement;
  ...
  it("auto-clears an additional-members entry when the same agent is picked as leader", async () => {
    ...
    // payload sent to addCrewMember (none — leader was the only pick).
    mocks.createCrew.mockResolvedValue(makeCrew({ leader_id: "agent-mine-2" }));
    ...
    await waitFor(() => {
      expect(mocks.createCrew).toHaveBeenCalledTimes(1);
    });
    // addCrewMember must NOT be called for the agent we promoted to leader.
    expect(mocks.addCrewMember).not.toHaveBeenCalled();
  });
  ...
  it("removes a member promoted to leader from selectedMembers so switching leader away does not resurrect it", async () => {
    ...
    mocks.createCrew.mockResolvedValue(makeCrew({ id: "sq-3", leader_id: "agent-mine-1" }));
    fireEvent.change(screen.getByPlaceholderText(/e\.g\. Frontend Team/i), {
      target: { value: "Swap Crew" },
    });
    fireEvent.click(getSubmitButton());

    await waitFor(() => {
      expect(mocks.createCrew).toHaveBeenCalledWith({
        name: "Swap Crew",
  ...
  it("on success with no additional members fires exactly one success toast and navigates", async () => {
    renderModal();
    fireEvent.change(screen.getByPlaceholderText(/e\.g\. Frontend Team/i), {
      target: { value: "Solo Crew" },
    });
    // Click MineAgentOne in the leader picker (first occurrence is leader picker row).
    fireEvent.click(firstMatch("MineAgentOne"));

    mocks.createCrew.mockResolvedValue(makeCrew({ id: "sq-1", leader_id: "agent-mine-1" }));
  ...
  it("on success with partial member failure shows success + warning toasts and still navigates", async () => {
    ...
    mocks.createCrew.mockResolvedValue(makeCrew({ id: "sq-2", leader_id: "agent-mine-1" }));
    mocks.addCrewMember
    ...
    await waitFor(() => {
      expect(mocks.addCrewMember).toHaveBeenCalledTimes(2);
    });
    await waitFor(() => {
      expect(mocks.toastSuccess).toHaveBeenCalledTimes(1);
    });
    expect(mocks.toastWarning).toHaveBeenCalledTimes(1);
    expect(mocks.navigationPush).toHaveBeenCalledWith("/test-ws/crews/sq-2");
  });

  it("on createCrew failure shows an error toast, does not navigate, and re-enables submit", async () => {
    renderModal();
    fireEvent.change(screen.getByPlaceholderText(/e\.g\. Frontend Team/i), {
      target: { value: "Boom Crew" },
    });
    fireEvent.click(firstMatch("MineAgentOne"));

    mocks.createCrew.mockRejectedValueOnce(new Error("server down"));
    ...
    // Submit button is re-enabled (textContent reads "Create Crew" again,
    // not "Creating...").
