...
vi.mock("@orchestra/core/issues/queries", () => ({
...
vi.mock("@orchestra/core/issues/stores/draft-store", () => ({
...
vi.mock("@orchestra/core/issues/stores/quick-create-store", () => ({
  useQuickCreateStore: (selector?: (state: typeof mockQuickCreateStore) => unknown) =>
    (selector ? selector(mockQuickCreateStore) : mockQuickCreateStore),
}));

vi.mock("@orchestra/core/issues/stores/issue-create-settings-store", () => ({
...
vi.mock("@orchestra/core/labels", () => ({
  useAttachLabelToIssue: () => ({ mutateAsync: mockAttachLabel }),
}));

vi.mock("@orchestra/core/properties", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@orchestra/core/properties")>();
  return {
    ...actual,
    useSetIssueProperty: () => ({
...
vi.mock("@orchestra/core/api", async () => {
  ...
  const { parseWithFallback } = await vi.importActual<typeof import("@orchestra/core/api/schema")>(
    "@orchestra/core/api/schema",
  );
  const { DuplicateIssueErrorBodySchema } = await vi.importActual<
    typeof import("@orchestra/core/api/schemas")
  >("@orchestra/core/api/schemas");
  return {
    api: {
...
vi.mock("@orchestra/ui/components/ui/switch", () => ({
  Switch: ({
    checked,
    onCheckedChange,
  }: {
...
vi.mock("@orchestra/ui/lib/utils", () => ({
