...
describe("Attachment — image dispatch", () => {
  it("record image renders <img> with hover toolbar (View/Download/Copy)", () => {
    ...
    // attachment.tsx for the rationale (ISS-3130 review follow-up).
  ...
  it("url-only image resolves to a record via context and uses its id for download", () => {
    ...
    // (ISS-3130 review). The raw stored url is the fallback for
  ...
  it("does not pick the raw CDN url when the server reports cdn_signed (ISS-3254)", () => {
  ...
  it("re-signs the inline media URL through getAttachment on token-mode clients (ISS-3254)", async () => {
  ...
  it("re-signs URL-only inline media when no resolver record is available (ISS-3254)", async () => {
  ...
  it("falls back to an authenticated byte fetch when the deployment has no signed URL (ISS-5445)", async () => {
  ...
  it("copies the durable URL, not the session-local object URL (ISS-5445)", async () => {
  ...
  it("does not pull bytes for non-image attachments (ISS-5445)", async () => {
  ...
  it("prefers the signed URL over a byte fetch when the server can presign (ISS-5445)", async () => {
  ...
  it("stable /api/attachments/<id>/download download_url falls through to the durable record.markdown_url for native <img> loadability (ISS-3192)", () => {
    // After ISS-3192, pickInlineMediaURL prefers `record.markdown_url`
  ...
  it("legacy backend (no markdown_url on record) still falls back to record.url", () => {
    // A backend old enough to predate ISS-3192 omits markdown_url; the
...
// ISS-3192 — Desktop quick-create: site-relative `/api/attachments/<id>/
...
describe("Attachment — absolutize site-relative URLs (ISS-3192)", () => {
  ...
  it("absolutizes the legacy site-relative /uploads/<key> when record.markdown_url is empty (legacy backend)", () => {
    // Legacy compat: a backend old enough to predate ISS-3192 omits
