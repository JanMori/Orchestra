...
/**
 * The coordinated-upload engine shared by every composer surface (ISS-5181, L2).
 *
 * Ownership inversion: an upload is owned by the module-level upload
 * coordinator (`@orchestra/core/drafts`), not by the React component that
...
import {
  ...
  type DraftUpload,
} from "@orchestra/core/drafts";
import { createSafeId } from "@orchestra/core/utils";
import { contentReferencesAttachment, type Attachment } from "@orchestra/core/types";
import {
  toUploadResult,
  type UploadContext,
  type UploadResult,
} from "@orchestra/core/hooks/use-file-upload";
import { MAX_FILE_SIZE } from "@orchestra/core/constants/upload";
...
export function useCoordinatedUploads(
  ...
  // a user who deletes the placeholder mid-upload means it, and ISS-5181's
            ...
            if (outcome.status === "uploaded") {
              if (target) {
                ...
                if (target.getUploads().some((u) => u.clientUploadId === clientUploadId)) {
                  ...
                  // Write-back (ISS-5181): the mount that started this upload
