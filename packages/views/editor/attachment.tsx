...
function normalize(
  ...
  return {
    ...
    // markdown stays on its existing path. See ISS-3130 review.
    //
    ...
    // See ISS-3192 — quick-create modal regressed because the freshly-
...
// `/uploads/<key>` URL.
//
// The current (post-ISS-3192) write path persists an absolute URL chosen
...
// matters for content written BEFORE ISS-3192 — those bodies still carry
...
//                             persistence (ISS-3192 / `buildMarkdownURL`):
//                             public CDN passthrough when the storage is
//                             public-readable, or `ORCHESTRA_PUBLIC_URL +
...
//     bucket (must-fix 2 from ISS-3192 review), except for the explicit
//     site-relative local upload path above.
//  5. `record.url` — legacy fallback for responses that omit
//     `markdown_url` (a backend old enough to predate ISS-3192).
