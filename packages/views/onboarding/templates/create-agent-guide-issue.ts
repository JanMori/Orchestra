...
const HELPER_AGENT_NAME = "Orchestra Helper";

/**
 * Skip path, issue 2/2: "Create your first Orchestra Agent".
 *
...
export const CREATE_AGENT_GUIDE_ISSUE_TITLE = {
  en: "Step 2 — Create your first Orchestra Agent",
  zh: "第 2 步 —— 创建你的第一个 Orchestra Agent",
  ko: "2단계 — 첫 Orchestra Agent 만들기",
  ja: "ステップ2 — 最初の Orchestra Agent を作成する",
} as const;
...
function enBody(installRuntimeMention: string): string {
  return `Once your runtime is online (see ${installRuntimeMention}), build your first agent — Orchestra Helper. The prompt below is pre-written; just copy.
...
Now create an issue (or reassign an existing one) → set assignee = Orchestra Helper → set status to **todo**. The runtime picks the task up within a few seconds and starts working. Watch progress in the issue's task panel.

## Where to go next

- **Skills** — reusable instruction packs you can attach to any agent.
- **Crews** — groups of agents that can be assigned together.
- **Autopilots** — scheduled or webhook-triggered runs.
- **Docs** — http://localhost:5001/docs.`;
}

function zhBody(installRuntimeMention: string): string {
  return `等运行时上线（见 ${installRuntimeMention}）之后，把第一个 agent —— Orchestra Helper —— 建出来。下面的提示词已经写好，直接复制即可。
...
接着创建一个任务（或把已有任务重新分派）→ 把 assignee 设成 Orchestra Helper → 状态切到 **todo**。运行时会在几秒内接走任务并开始工作。在任务的 task 面板里看进度。

## 接下来去哪

- **Skills** —— 可复用的指令包，可挂到任何 agent 上。
- **Crews** —— 可一起被分派的一组 agent。
- **Autopilots** —— 定时或 webhook 触发的运行。
- **文档** —— http://localhost:5001/docs。`;
}

function koBody(installRuntimeMention: string): string {
  return `runtime이 online 상태가 되면(${installRuntimeMention} 참고), 첫 agent인 Orchestra Helper를 만드세요. 아래 prompt는 미리 작성되어 있으니 그대로 복사하면 됩니다.
...
이제 태스크를 만들거나 기존 태스크를 다시 배정한 뒤 assignee를 Orchestra Helper로 설정하고 status를 **todo**로 바꾸세요. runtime이 몇 초 안에 작업을 가져가 실행을 시작합니다. 진행 상황은 태스크의 task panel에서 볼 수 있습니다.

## 다음에 볼 곳

- **Skills** — 어떤 agent에도 붙일 수 있는 재사용 instruction pack입니다.
- **Crews** — 함께 배정할 수 있는 agent 그룹입니다.
- **Autopilots** — 예약 또는 webhook으로 실행되는 작업입니다.
- **Docs** — http://localhost:5001/docs.`;
}

function jaBody(installRuntimeMention: string): string {
  return `runtime が online になったら(${installRuntimeMention} を参照)、最初の agent である Orchestra Helper を作りましょう。下の prompt はあらかじめ書いてあるので、そのままコピーするだけです。
...
次にタスクを作る(または既存のタスクを割り当て直す)→ assignee を Orchestra Helper にする → status を **todo** にします。runtime が数秒以内にタスクを受け取って作業を始めます。進捗はタスクの task panel で確認できます。

## 次に見る場所

- **Skills** — どの agent にも付けられる、再利用可能な instruction パックです。
- **Crews** — 一緒に割り当てられる agent のグループです。
