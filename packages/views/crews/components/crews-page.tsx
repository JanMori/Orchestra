...
import {
  useCrewsViewStore,
  CREW_SCOPES,
  CREW_DEFAULT_HIDDEN_COLUMNS,
  type CrewColumnKey,
  type CrewListFilters,
  type CrewsScope,
  type CrewSortField,
} from "@orchestra/core/crews/stores";
import type { Agent, MemberWithUser, Crew } from "@orchestra/core/types";
...
// Column template — the simplest member of the ListGrid family (crews are
...
const COLUMN_WIDTHS: Record<CrewColumnKey, number> = {
  members: 120,
  creator: 144,
  created: 104,
};
...
function columnTrackVars(
  isVisible: (key: CrewColumnKey) => boolean,
  showActions: boolean,
): React.CSSProperties {
  const width = (key: CrewColumnKey) =>
    isVisible(key) ? `${COLUMN_WIDTHS[key]}px` : "0px";
  const minWidth =
    FIXED_TRACKS_WIDTH +
    (Object.keys(COLUMN_WIDTHS) as CrewColumnKey[]).reduce(
...
function CrewAvatar({ crew }: { crew: Crew }) {
...
function NameCell({ crew }: { crew: Crew }) {
  return (
...
// is agent-only.
function MembersCell({ crew }: { crew: Crew }) {
  const preview = crew.member_preview ?? [];
  const count = crew.member_count ?? preview.length;
  if (count === 0) {
    return (
...
// once archived a crew is gone from the UI.
// ---------------------------------------------------------------------------

function ArchiveCrewDialog({
  crew,
  open,
  onOpenChange,
}: {
  ...
  const archive = useMutation({
    mutationFn: () => api.deleteCrew(crew.id),
    onSuccess: () => {
...
function CrewRowActions({ crew }: { crew: Crew }) {
  const { t } = useT("crews");
...
function CrewListHeader({
  ...
  onSort: (field: CrewSortField) => void;
  isColVisible: (key: CrewColumnKey) => boolean;
}) {
  const { t } = useT("crews");
  const sorted = (field: CrewSortField) =>
...
const COLUMN_KEYS: CrewColumnKey[] = ["members", "creator", "created"];
const SORT_FIELDS: CrewSortField[] = ["name", "members", "created"];
...
function CrewListToolbar({
  ...
  scope: CrewsScope;
  onScopeChange: (scope: CrewsScope) => void;
  scopeCounts: Record<CrewsScope, number>;
  filters: CrewListFilters;
  onToggleFilter: (key: keyof CrewListFilters, value: string) => void;
  onClearFilters: () => void;
  ...
  const SCOPE_LABELS: Record<CrewsScope, string> = {
    mine: t(($) => $.scope.mine),
    all: t(($) => $.scope.all),
  };
  const SORT_LABELS: Record<CrewSortField, string> = {
    name: t(($) => $.page.table.name),
    members: t(($) => $.page.table.members),
    created: t(($) => $.page.table.created),
  };
  const COLUMN_LABELS: Record<CrewColumnKey, string> = {
    members: t(($) => $.page.table.members),
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-auto">
            <DropdownMenuRadioGroup
              value={scope}
              onValueChange={(value) => onScopeChange(value as CrewsScope)}
            >
              {CREW_SCOPES.map((s) => (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-auto">
                  <DropdownMenuRadioGroup
                    value={sortField}
                    onValueChange={(v) =>
                      onSortFieldChange(v as CrewSortField)
                    }
                  >
...
export function CrewsPage() {
  ...
  const scope = useCrewsViewStore((s) => s.scope);
  const setScope = useCrewsViewStore((s) => s.setScope);
  const sortField = useCrewsViewStore((s) => s.sortField);
  const sortDirection = useCrewsViewStore((s) => s.sortDirection);
  const hiddenColumns = useCrewsViewStore((s) => s.hiddenColumns);
  const handleSort = useCrewsViewStore((s) => s.toggleSort);
  const handleSortFieldSelect = useCrewsViewStore((s) => s.setSortField);
  const setSortDirection = useCrewsViewStore((s) => s.setSortDirection);
  const toggleColumn = useCrewsViewStore((s) => s.toggleColumn);
  const filters = useCrewsViewStore((s) => s.filters);
  const toggleFilter = useCrewsViewStore((s) => s.toggleFilter);
  const clearFilters = useCrewsViewStore((s) => s.clearFilters);

  const isColVisible = (key: CrewColumnKey) => !hiddenColumns.includes(key);

  const scopeCounts = useMemo<Record<CrewsScope, number>>(() => {
    let mine = 0;
    if (currentUser) {
      for (const s of crews) if (s.creator_id === currentUser.id) mine++;
    }
    return { mine, all: crews.length };
  }, [crews, currentUser]);

  // Rows within the current scope, unfiltered — filter option lists + the
  // "n / total" denominator derive from this.
  const scopeRows = useMemo<Crew[]>(() => {
    return crews.filter((s) => {
      if (scope === "mine") {
        return !!currentUser && s.creator_id === currentUser.id;
      }
      return true;
    });
  }, [crews, scope, currentUser]);
      ...
      <CollectionPageHeader
        ...
        actions={
          <CollectionPageHeaderAction
            ...
            onClick={() => useModalStore.getState().open("create-crew")}
          />
        }
      />
              ...
              {rows.length === 0 ? (
                ...
                rows.map((crew) => (
                  <ListGridRow
                    ...
                    {...rowLink(p.crewDetail(crew.id))}
                  >
                    <NameCell crew={crew} />
                    <LeaderCell
                      leaderId={crew.leader_id}
                      leader={agentsById.get(crew.leader_id)}
                    />
                    {isColVisible("members") ? (
                      <MembersCell crew={crew} />
                    ) : (
                    ...
                    {isColVisible("creator") ? (
                      <ListGridCell className="hidden gap-1.5 @2xl:flex">
                        <ActorAvatar
                          actorType="member"
                          actorId={crew.creator_id}
                          size="sm"
                        />
                        <span className="min-w-0 truncate text-caption text-muted-foreground">
                          {membersById.get(crew.creator_id)?.name ??
                            crew.creator_id.slice(0, 8)}
                        </span>
                      </ListGridCell>
                    ) : (
                      <ListGridCell className="hidden px-0 @2xl:flex" />
                    ...
                    <ListGridCell className="justify-end px-0">
                      {isWorkspaceAdmin ||
                      (!!currentUser && crew.creator_id === currentUser.id) ? (
                        <CrewRowActions crew={crew} />
                      ) : null}
                    </ListGridCell>
                  </ListGridRow>
                ))
    ...
    <div className="min-h-0 flex-1 overflow-auto @container">
      <ListGrid
        ...
        style={columnTrackVars(
          (key) => !CREW_DEFAULT_HIDDEN_COLUMNS.includes(key),
          true,
        )}
      >
        <ListGridHeader>
