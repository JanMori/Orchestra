...
import { getShortcut, shortcutMatchesEvent } from "@orchestra/core/shortcuts";
import { useTimeAgo } from "../../i18n";
import { agentListOptions, memberListOptions, crewMemberStatusOptions, workspaceKeys } from "@orchestra/core/workspace/queries";
...
import type { Crew, CrewMember, CrewMemberStatus, CrewMemberStatusValue, Agent, MemberWithUser } from "@orchestra/core/types";
...
export function CrewDetailPage() {
  ...
  const crewId = pathname.split("/").pop() ?? "";

  const { data: crew, refetch: refetchCrew } = useQuery<Crew>({
    queryKey: [...workspaceKeys.crews(wsId), crewId],
    queryFn: () => api.getCrew(crewId),
    enabled: !!workspace?.id && !!crewId,
  });

  const { data: members = [], refetch: refetchMembers } = useQuery<CrewMember[]>({
    queryKey: [...workspaceKeys.crews(wsId), crewId, "members"],
    queryFn: () => api.listCrewMembers(crewId),
    enabled: !!workspace?.id && !!crewId,
  });

  // Per-crew working/idle/offline + active-issue snapshot. WS task / agent /
  // daemon events invalidate this via use-realtime-sync; the staleTime is a
  // tab-focus safety net. Indexed by member_id so CrewMembersTab can look up
  // its row in O(1).
  const { data: memberStatusResp } = useQuery({
    ...crewMemberStatusOptions(wsId, crewId),
    enabled: !!workspace?.id && !!crewId,
  });
  const memberStatusById = useMemo(() => {
    const map = new Map<string, CrewMemberStatus>();
  ...
  // Per-crew management gate: workspace owner/admin manage every crew; the
  ...
  const updateCrewMut = useMutation({
    mutationFn: (data: { name?: string; description?: string; instructions?: string; avatar_url?: string; leader_id?: string }) => api.updateCrew(crewId, data),
    onSuccess: () => {
      refetchCrew();
      refetchMembers();
      queryClient.invalidateQueries({ queryKey: workspaceKeys.crews(wsId) });
    },
  });

  const addMemberMut = useMutation({
    mutationFn: (input: { type: "agent" | "member"; id: string; role?: string }) =>
      api.addCrewMember(crewId, {
  ...
  const removeMemberMut = useMutation({
    mutationFn: (m: CrewMember) => api.removeCrewMember(crewId, { member_type: m.member_type, member_id: m.member_id }),
  ...
  const updateRoleMut = useMutation({
    mutationFn: (input: { member: CrewMember; role: string }) =>
      api.updateCrewMemberRole(crewId, {
  ...
  const setLeaderMut = useMutation({
    mutationFn: (agentId: string) => api.updateCrew(crewId, { leader_id: agentId }),
    onSuccess: () => {
      refetchCrew();
      refetchMembers();
      queryClient.invalidateQueries({ queryKey: workspaceKeys.crews(wsId) });
  ...
  const deleteMut = useMutation({
    mutationFn: () => api.deleteCrew(crewId),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: workspaceKeys.crews(wsId) }); push(p.crews()); toast.success("Crew archived"); },
    onError: (err) =>
      toast.error(err instanceof Error && err.message ? err.message : "Failed to archive crew"),
  });
  ...
  const isLeader = (m: CrewMember) => m.member_type === "agent" && crew.leader_id === m.member_id;
  const isArchived = (m: CrewMember) =>
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      <BreadcrumbHeader
        segments={[{ href: p.crews(), label: t(($) => $.page.title) }]}
        leaf={
          <>
            <CrewHeaderAvatar crew={crew} initials={initials} />
            <h1 className="truncate text-body font-medium text-foreground">{crew.name}</h1>
          </>
        }
        actions={
          canManage ? (
      ...
      <div className="flex flex-1 min-h-0 flex-col gap-3 overflow-y-auto p-3 md:grid md:grid-cols-[280px_minmax(0,1fr)] md:gap-4 md:overflow-hidden md:p-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <CrewDetailInspector
          ...
          leaderName={getEntityName("agent", crew.leader_id)}
          creatorName={getEntityName("member", crew.creator_id)}
          canManage={canManage}
          onUploadAvatar={(url) => updateCrewMut.mutateAsync({ avatar_url: url })}
          onRename={async (next) => { await updateCrewMut.mutateAsync({ name: next.trim() }); }}
          onUpdateDescription={async (next) => { await updateCrewMut.mutateAsync({ description: next }); }}
        />
        ...
        <CrewOverviewPane
          ...
          onCreateAgentClick={canManage ? () => push(`${p.newAgent()}?crew=${encodeURIComponent(crewId)}`) : undefined}
          ...
          onSaveInstructions={async (next) => { await updateCrewMut.mutateAsync({ instructions: next }); toast.success("Instructions saved"); }}
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t(($) => $.archive_dialog.description, { name: crew.name })}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
...
// to the Users icon when no custom avatar is set so the crew still has a
// recognisable glyph in the breadcrumb strip.
function CrewHeaderAvatar({ crew, initials }: { crew: Crew; initials: string }) {
  if (!crew.avatar_url) {
  ...
  return (
    <ActorAvatarBase
      name={crew.name}
      initials={initials}
      avatarUrl={resolvePublicFileUrl(crew.avatar_url)}
      size="sm"
      className="shrink-0"
    />
  );
...
// Read-only 64px avatar for viewers who can't manage the crew — same visual
// as the editable control's resting state but without the click/upload
// affordance.
function CrewStaticAvatar({ crew, initials }: { crew: Crew; initials: string }) {
  return (
...
// 2) optionally describe the role they'll play in this crew. Description
//    lives here (not on the picker) because role is per-crew context that
...
// ---------------------------------------------------------------------------
// CrewDetailInspector — left 320px column, mirrors AgentDetailInspector.
// Holds identity (avatar / name / description) + leader / member count /
// timestamps. All inline-editable.
// ---------------------------------------------------------------------------
function CrewDetailInspector({
  crew,
  memberCount,
  leaderName,
  creatorName,
  ...
  // no rename/description popovers) — the viewer can read the crew but not
      ...
      <div className="flex flex-col gap-3 border-b px-5 pb-5 pt-5">
        {canManage ? (
          <>
            <AvatarUploadControl
              variant="crew"
              value={crew.avatar_url ?? null}
              name={crew.name}
              size={64}
              onUploaded={onUploadAvatar}
            />
            <div className="flex flex-col gap-1">
              <CrewNameEditor value={crew.name} onSave={onRename} />
              <CrewDescriptionEditor
          ...
          <>
            <CrewStaticAvatar crew={crew} initials={initials} />
            <div className="flex flex-col gap-1">
              <span className="text-title font-semibold leading-tight">{crew.name}</span>
              {crew.description ? (
        ...
        <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5">
          <InspectorRow label="Leader">
            <span className="flex min-w-0 items-center gap-1.5">
              <ActorAvatar actorType="agent" actorId={crew.leader_id} size="xs" />
          ...
          <InspectorRow label="Created by">
            <span className="flex min-w-0 items-center gap-1.5">
              <ActorAvatar actorType="member" actorId={crew.creator_id} size="xs" />
              <span className="truncate">{creatorName}</span>
            </span>
          </InspectorRow>
          <InspectorRow label="Created">
            <span className="text-muted-foreground">{timeAgo(crew.created_at)}</span>
          </InspectorRow>
          <InspectorRow label="Updated">
            <span className="text-muted-foreground">{timeAgo(crew.updated_at)}</span>
          </InspectorRow>
        </div>
      </div>
    </aside>
...
function CrewDescriptionEditor({
  value,
  onSave,
}: {
  value: string;
...
function CrewDescriptionEditorBody({
  initialValue,
  onSave,
  onClose,
}: {
...
// ---------------------------------------------------------------------------
// CrewOverviewPane — right column with two tabs (Members | Instructions).
// Mirrors AgentOverviewPane: dirty-guard via AlertDialog when switching tabs
// with unsaved Instructions.
// ---------------------------------------------------------------------------
type CrewDetailTab = "members" | "instructions";

const crewDetailTabs: { id: CrewDetailTab; label: string; icon: typeof FileText }[] = [
  { id: "members", label: "Members", icon: Users },
  { id: "instructions", label: "Instructions", icon: FileText },
];

function CrewOverviewPane({
  crew,
  members,
  memberStatusById,
  canManage,
  ...
  crew: Crew;
  members: CrewMember[];
  memberStatusById: Map<string, CrewMemberStatus>;
  ...
  // Save). See canManageCrew in server/internal/handler/crew.go.
  canManage: boolean;
  isLeader: (m: CrewMember) => boolean;
  isArchived: (m: CrewMember) => boolean;
  getEntityName: (type: string, id: string) => string;
  onAddMemberClick: () => void;
  // Optional — only passed when the current user can manage the crew
  ...
  onRemoveMember: (m: CrewMember) => void;
  onUpdateRole: (m: CrewMember, role: string) => Promise<void>;
  onSaveInstructions: (next: string) => Promise<void>;
  setLeaderPending: boolean;
}) {
  const { t } = useT("crews");
  const [activeTab, setActiveTab] = useState<CrewDetailTab>("members");
  const [activeDirty, setActiveDirty] = useState(false);
  const [pendingTab, setPendingTab] = useState<CrewDetailTab | null>(null);

  const requestTabChange = (next: CrewDetailTab) => {
...
// Visual config for the five crew member status buckets. Mirrors
...
const CREW_STATUS_DOT_CLASS: Record<CrewMemberStatusValue, string> = {
  working: "bg-success",
...
function CrewMembersTab({
  ...
  getEntityName: (type: string, id: string) => string;
  onAddMemberClick: () => void;
  // Hidden for viewers who can't manage — see CrewOverviewPane.
  onCreateAgentClick?: () => void;
  onSetLeader: (agentId: string) => void;
  onRemoveMember: (m: CrewMember) => void;
  onUpdateRole: (m: CrewMember, role: string) => Promise<void>;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="space-y-2">
        {members.map((m) => {
          ...
          const dotClass =
            statusValue && statusValue in CREW_STATUS_DOT_CLASS
              ? CREW_STATUS_DOT_CLASS[statusValue as keyof typeof CREW_STATUS_DOT_CLASS]
              : null;
          const statusLabel =
...
// Save button. The crew leader's prompt picks these up at task claim time
// (server/internal/handler/daemon.go).
function CrewInstructionsTab({
  crew,
  canManage,
  onSave,
  onDirtyChange,
  ...
  const { t } = useT("crews");
  const [value, setValue] = useState(crew.instructions ?? "");
  const [saving, setSaving] = useState(false);
  const isDirty = value !== (crew.instructions ?? "");

  useEffect(() => {
    setValue(crew.instructions ?? "");
  }, [crew.id, crew.instructions]);
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      ...
      {/* When the viewer can't manage the crew, the editor is wrapped in a
