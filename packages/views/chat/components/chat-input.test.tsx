...
import type { DraftUpload } from "@orchestra/core/drafts";
...
// DraftUpload entries (ISS-5181 L2).
...
// ISS-4864: an uncreated chat has ONE draft per workspace. `selectedAgentId`
...
describe("ChatInput @ context wiring", () => {
  it("configures chat @ with current/recent issue/project context", () => {
    const contextItems = [
      { id: "issue-1", label: "ISS-1", type: "issue" as const, group: "current" as const },
    ];
...
describe("ChatInput project context", () => {
  ...
  it("locks the project control while a send is in flight so a mid-send switch cannot retarget the session", async () => {
    ...
    // (ISS-5150). The control must stay locked for the whole send, not only
...
describe("ChatInput attachment wiring", () => {
  ...
  it("binds attachment_ids when the upload's markdownLink differs from its link (ISS-3130 regression)", async () => {
...
describe("ChatInput commit handoff", () => {
  ...
  it("scrubs the editor and clears the draft on a normal commit", async () => {
    ...
    // rather than being dropped (ISS-5181 follow-up).
