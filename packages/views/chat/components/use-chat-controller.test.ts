...
import type { ChatMessage, ChatMessagesPage, ChatPendingTask } from "@orchestra/core/types";
import {
...
// dropped as dangling. Post-ISS-5181 the send is await-then-render, so the
...
// ISS-4864: the new-chat composer is one box per workspace, so the selected
...
// Regression (ISS-5150 review): switching an existing session to another
