...
//                    pre-ISS-3130 contract for non-markdown callers
...
//                    `<ORCHESTRA_PUBLIC_URL>/api/attachments/<id>/download`).
...
//
// ISS-3130 introduced the persisted-image regression by collapsing
// these two semantics into a single `link` field; ISS-3192 followed up
...
//      modern contract introduced in ISS-3192; the server (`buildMarkdownURL`)
//      decides whether to emit a public CDN URL or an absolute API
//      endpoint pinned to `ORCHESTRA_PUBLIC_URL` based on the deployment.
//   2. `attachmentDownloadPath(att.id)` — site-relative legacy shape,
//      retained for compatibility with backends old enough to predate
//      ISS-3192. Web's Next rewrite makes this load; desktop / mobile
...
// coordinated-upload path (ISS-5181), which runs the request through the
...
export function useFileUpload(
  ...
  // their attachment ids never to be bound (ISS-3339).
  //
