...
export function initAnalytics(config: AnalyticsConfig | null | undefined): boolean {
  ...
  posthog.init(config.key, {
    ...
    // identical `$exception` events per session (ISS-3331). The fingerprint is
...
export interface CaptureEventOptions {
   ...
   * matters (ISS-4115: three deterministic hangs, zero events delivered).
 ...
 * `@orchestra/ui` ErrorBoundary can opt in by passing `onError={captureException}`
