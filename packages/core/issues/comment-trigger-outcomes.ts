...
const MENTION_MARKUP_SOURCE =
  "\\[@?(.+?)\\]\\(mention:\\/\\/(member|agent|crew|issue|all)\\/([0-9a-fA-F-]+|all)\\)";
...
// (ISS-4525 §2). The create/edit responses are not fully schema-parsed, so the
...
// as success (ISS-4525; mirrors the Run now whitelist).
const HANDLED_TRIGGER_STATUSES = new Set(["queued", "coalesced", "deferred"]);

// The explicit @agent / @crew mentions that did NOT clearly trigger, so the
