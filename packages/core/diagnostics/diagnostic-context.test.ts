...
describe("bucketDiagnosticPath", () => {
  ...
  it("templates every workspace detail route", () => {
    expect(bucketDiagnosticPath("/acme/issues/ISS-5345")).toBe("/:slug/issues/:id");
    ...
    expect(bucketDiagnosticPath("/acme/crews/sq.4")).toBe("/:slug/crews/:id");
  ...
  it("templates ids that look nothing like ids", () => {
    for (const [path, expected] of [
      ...
      ["/acme/crews/Platform Team", "/:slug/crews/:id"],
  ...
  it("drops query string and hash — they can carry resource ids", () => {
    expect(bucketDiagnosticPath("/acme/issues?issue=ISS-1#comment-3")).toBe(
      "/:slug/issues",
    );
  });
  ...
  it("masks an unknown route instead of passing segments through", () => {
    expect(bucketDiagnosticPath("/acme/issues/ISS-1/secret-tab")).toBe(
      "/:slug/issues/*",
    );
