 ...
 * (ISS-4864). Created sessions keep their own slot, keyed by session id.
 */
export const DRAFT_NEW_SESSION = "__new__";

/** Pre-ISS-4864 per-agent new-chat slots, shaped `__new__:<agentId>`. */
...
function readDraftAttachments(storage: StorageAdapter, key: string): Record<string, DraftUpload[]> {
  ...
  try {
    ...
    for (const [draftKey, value] of Object.entries(parsed)) {
      ...
      // Normalize on every load (ISS-5181 L2): bare Attachment rows persisted
