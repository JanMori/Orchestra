...
// The pre-ISS-4864 scheme kept one new-chat draft per agent, in `__new__:<id>`
...
describe("chat store — legacy per-agent new-chat draft migration", () => {
  ...
  it("migrates the matching attachments with the text, not another agent's", () => {
    ...
    // Legacy bare Attachment rows load as `uploaded` entries (ISS-5181 L2).
    expect(
      store
        .getState()
...
// Coordinator-owned upload lifecycle in the draft slots (ISS-5181 L2).
