...
// Crew list responses carry lightweight membership previews used by hover
...
const CrewMemberPreviewSchema = z.object({
...
export const CrewSchema = z.object({
  ...
  member_preview: z.array(CrewMemberPreviewSchema).default([]),
}).loose();

export const CrewListSchema = z.array(CrewSchema);
export const EMPTY_CREW_LIST: Crew[] = [];
...
// Crew member status — backs the Crew detail page's Members tab. status
// is `string | null` (not the narrow `CrewMemberStatusValue` union) so a
// new server-side status doesn't fail the parse; the UI defaults to a
// neutral pill for unknown values.
const CrewActiveIssueBriefSchema = z.object({
  issue_id: z.string(),
  identifier: z.string(),
  title: z.string(),
...
const CrewMemberStatusSchema = z.object({
  member_type: z.string(),
  member_id: z.string(),
  status: z.string().nullable().optional().transform((v) => v ?? null),
  active_issues: z.array(CrewActiveIssueBriefSchema).default([]),
  last_active_at: z.string().nullable().optional().transform((v) => v ?? null),
}).loose();

export const CrewMemberStatusListResponseSchema = z.object({
  members: z.array(CrewMemberStatusSchema).default([]),
}).loose();

export const EMPTY_CREW_MEMBER_STATUS_LIST = { members: [] };
