...
import { agentListOptions, crewListOptions } from "../workspace/queries";
...
// list may show incomplete results (e.g. missing crews).
//
// useRealtimeSync (WS task / agent / daemon / crew invalidations) and the
...
export function useWorkspacePresencePrefetch(wsId: string | undefined): void {
  ...
  useQuery({ ...crewListOptions(wsId ?? ""), enabled: !!wsId });
}
