...
describe("failureClassOf", () => {
  it("maps the reasons an operator acts on to distinct classes", () => {
    ...
    // ISS-5370: the run never reached the model provider, so this belongs
    ...
    // ISS-5722: the daemon and the provider are both healthy — codex could
  ...
  it("keeps pre-ISS-1949 coarse reasons countable", () => {
