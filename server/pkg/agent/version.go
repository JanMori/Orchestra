...
// the orchestra CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `orchestra issue create` failure — see PR
...
var (
	ErrCLIVersionMissing = errors.New("orchestra CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("orchestra CLI version is below required minimum")
)
