...
// TestDetectVersionTimesOutOnHang guards ISS-3812: a CLI whose `--version`
...
func TestDetectVersionTimesOutOnHang(t *testing.T) {
	...
	case <-time.After(10 * time.Second):
		t.Fatal("DetectVersion did not return: version probe is unbounded (regression of ISS-3812)")
	}
}
...
func TestRunContextPositiveTimeoutHasDeadline(t *testing.T) {
	...
	// absolute cap operators can still set via ORCHESTRA_AGENT_TIMEOUT).
