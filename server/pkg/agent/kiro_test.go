...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_comment_done","update":{"type":"ToolCall","toolCallId":"tc-comment","name":"Shell","status":"pending","parameters":{"command":"orchestra issue comment add issue-1 --content-file ./reply.md"}}}}\n'
...
func TestKiroIssueCommentAddCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{"orchestra issue comment add issue-1 --content-file ./reply.md", true},
		{"./orchestra issue comment add issue-1 --content-file ./reply.md", true},
		{"/usr/local/bin/orchestra issue comment add issue-1 --content-file ./reply.md", true},
		{"ORCHESTRA_TOKEN=x orchestra issue comment add issue-1 --content-file ./reply.md", true},
		{"FOO=1 BAR=2 ./orchestra issue comment add issue-1", true},
		{`sh -c "orchestra issue comment add issue-1 --content-file ./reply.md"`, true},
		{`bash -c 'orchestra issue comment add issue-1'`, true},
		{`/bin/sh -c "orchestra issue comment add issue-1"`, true},
		{"orchestra issue get issue-1", false},
		{"echo orchestra issue comment add issue-1", false},
		{`sh -c "echo orchestra issue comment add issue-1"`, false},
		{"FOO=bar", false},
		{"", false},
	}
	for _, tt := range tests {
...
// `orchestra issue comment add` must be recognized from its command payload
...
func TestKiroIssueCommentAddToolIgnoresToolTitle(t *testing.T) {
	...
	for _, tt := range tests {
		msg := Message{
			Type:  MessageToolUse,
			Tool:  tt.tool,
			Input: map[string]any{"command": "orchestra issue comment add issue-1 --content-file ./reply.md"},
		}
...
func TestKiroBackendDoesNotCompleteRunningCommentAddWithoutResult(t *testing.T) {
	...
	result := runKiroCloseErrorScript(t, fakeKiroACPRunningToolCloseErrorScript(
		"execute_bash",
		"orchestra issue comment add issue-1 --content-file ./reply.md",
	))
	if result.Status != "failed" {
...
// ISS-4860). Three things about this shape broke the older guards:
...
// #5509 regression against the real captured wire shape: a `orchestra issue
...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_two","update":{"type":"ToolCall","toolCallId":"tc-1","name":"terminal","status":"pending","parameters":{"command":"orchestra issue comment add issue-1 --content-file ./progress.md"}}}}\n'
      ...
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_two","update":{"type":"ToolCall","toolCallId":"tc-2","name":"terminal","status":"pending","parameters":{"command":"orchestra issue comment add issue-1 --content-file ./final.md"}}}}\n'
...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_failed","update":{"type":"ToolCall","toolCallId":"tc-fail","name":"execute_bash","status":"pending","parameters":{"command":"orchestra issue comment add issue-1 --content-file ./reply.md"}}}}\n'
