...
INSERT INTO autopilot_run (
    ...
    $6, $7,
    $8
) RETURNING id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, crew_id, planned_at, webhook_delivery_id
`
...
SELECT id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, crew_id, planned_at, webhook_delivery_id FROM autopilot_run
WHERE id = $1
`
...
const pauseAutopilotsByUnrunnableCrew = `-- name: PauseAutopilotsByUnrunnableCrew :many
UPDATE autopilot
SET status = 'paused',
    pause_reason = 'agent_runtime_required',
    updated_at = now()
...
func (q *Queries) PauseAutopilotsByUnrunnableCrew(ctx context.Context, crewID pgtype.UUID) ([]Autopilot, error) {
	rows, err := q.db.Query(ctx, pauseAutopilotsByUnrunnableCrew, crewID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
