...
-- platform-agnostic channel_* tables (ISS-3515, parent ISS-3506). Each
...
--
--   * NO foreign keys and NO cascades (ISS-3515 §4). The lark_* tables
...
--     A one-time ORCHESTRA_LARK_HUB_DISABLED park-switch existed during the
...
--     since been removed (ISS-3515); this note is kept as history. Rollback to
...
CREATE TABLE channel_installation (
    ...
    -- that is an application-layer rule (ISS-3515 §4), not a DB constraint.
