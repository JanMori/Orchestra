...
-- was deemed unacceptable. ISS-972 closed that loop on prd:
--
...
BEGIN
  ...
  IF conflict_count > 0 THEN
    RAISE EXCEPTION 'Found % workspace(s) with slugs that collide with the legacy reserved-slug audit set: %. Rename or delete before deploying (see ISS-972 for the playbook).', conflict_count, conflict_list;
  END IF;
END $$;
