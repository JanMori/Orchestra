...
// `TASK-123` already covers every human use case for an issue reference, the
	...
	if _, err := normalizeUUIDPrefix(trimmed); err == nil {
		return resolvedID{}, fmt.Errorf(
			"issue ref %q looks like a short UUID prefix; short prefixes are no longer supported for issues. "+
				"Use the issue key (e.g. TASK-123) shown by `orchestra issue list`, or pass the full UUID (run a list command with --full-id to copy it)",
			input,
		)
	}
	return resolvedID{}, fmt.Errorf(
		"issue ref %q is not a recognized issue reference; use the issue key (e.g. TASK-123) shown by `orchestra issue list`, or pass the full UUID",
		input,
	)
}
...
func resolveTaskRunID(ctx context.Context, client *cli.APIClient, issueID, input string) (resolvedID, error) {
	...
	if strings.TrimSpace(issueID) == "" {
		return resolvedID{}, fmt.Errorf("short task run prefixes require --issue <issue-id>; pass a full task UUID or run `orchestra issue runs <issue-id> --full-id`")
	}
...
func (l actorDisplayLookup) loadCrews() {
	if l.state == nil || l.state.crewsLoaded {
		return
	}
	l.state.crewsLoaded = true
	l.state.crews = map[string]string{}
	if l.client == nil || l.client.WorkspaceID == "" {
		return
	}
	var crews []map[string]any
	if err := l.client.GetJSON(l.ctx, "/api/crews", &crews); err == nil {
		for _, s := range crews {
			if id := strVal(s, "id"); id != "" {
				l.state.crews[id] = strVal(s, "name")
			}
		}
	}
}
...
func (l actorDisplayLookup) actor(actorType, id string) string {
	...
	case "crew":
		l.loadCrews()
		if l.state != nil && l.state.crews != nil {
			if name := l.state.crews[id]; name != "" {
				return "crew:" + name
			}
		}
	}
