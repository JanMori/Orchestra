...
var workspaceCreateCmd = &cobra.Command{
	...
	Long: "Creates a new workspace and adds you as its owner. Both --name and " +
		...
		"this profile — run 'orchestra workspace switch <slug>' afterward if you " +
		"want subsequent commands to target the new workspace.",
	Example: "  orchestra workspace create --name \"Support Team\" --slug support-team --issue-prefix SUP",
	Args:    cobra.NoArgs,
...
var workspaceSwitchCmd = &cobra.Command{
	...
	Long: "Sets the default workspace for the current profile after verifying you " +
		...
		"For low-level use, 'orchestra config set workspace_id <id>' writes the " +
...
func fetchWorkspaces(ctx context.Context, cmd *cobra.Command) ([]workspaceSummary, error) {
	...
	if token == "" {
		return nil, fmt.Errorf("not authenticated: run 'orchestra login' first")
	}
...
func runWorkspaceList(cmd *cobra.Command, _ []string) error {
	...
	if currentID != "" {
		fmt.Fprintln(os.Stderr, "\n* = current default workspace (use 'orchestra workspace switch <id|slug|prefix>' to change)")
	} else {
		fmt.Fprintln(os.Stderr, "\nNo default workspace set. Use 'orchestra workspace switch <id|slug|prefix>' to pick one.")
	}
...
func resolveWorkspaceByIDOrSlug(workspaces []workspaceSummary, target string) (workspaceSummary, error) {
	...
	return workspaceSummary{}, fmt.Errorf("workspace %q not found or you do not have access; run 'orchestra workspace list' to see options", target)
}
