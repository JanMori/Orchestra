...
func newCrewMemberSetRoleTestCmd() *cobra.Command {
...
func TestCrewMemberSetRoleCommandIsRegistered(t *testing.T) {
	cmd, _, err := crewMemberCmd.Find([]string{"set-role", "crew-123"})
	if err != nil {
...
func TestRunCrewMemberSetRolePatchesRole(t *testing.T) {
	...
	cmd := newCrewMemberSetRoleTestCmd()
	_ = cmd.Flags().Set("member-id", "member-456")
	_ = cmd.Flags().Set("member-type", "agent")
	_ = cmd.Flags().Set("role", "reviewer")
	_ = cmd.Flags().Set("output", "json")

	if err := runCrewMemberSetRole(cmd, []string{"crew-123"}); err != nil {
		t.Fatalf("runCrewMemberSetRole: %v", err)
	}
	if gotMethod != http.MethodPatch {
		t.Fatalf("method = %s, want PATCH", gotMethod)
	}
	if gotPath != "/api/crews/crew-123/members/role" {
		t.Fatalf("path = %q, want /api/crews/crew-123/members/role", gotPath)
	}
...
func TestRunCrewMemberSetRoleValidatesRequiredFlags(t *testing.T) {
	cmd := newCrewMemberSetRoleTestCmd()
	if err := runCrewMemberSetRole(cmd, []string{"crew-123"}); err == nil {
