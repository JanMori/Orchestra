...
func TestFindLocalPathLinksAllowsLegitimateContent(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			body: "see [ISS-1](/acme/issues/ISS-1) and [inbox](/issues/123)",
		},
		{
...
func TestGuardLocalPathLinksOnlyFiresInAgentContext(t *testing.T) {
	...
	t.Run("human PAT context is never linted", func(t *testing.T) {
		// No ORCHESTRA_AGENT_ID / ORCHESTRA_TASK_ID: a person running the CLI. Their
	...
	t.Run("agent context with a clean body passes", func(t *testing.T) {
		withAgentContext(t)
		if err := guardLocalPathLinks("all good — see [ISS-1](/acme/issues/ISS-1)", "comment body", "hint"); err != nil {
...
func TestGuardLocalPathLinksHintIsPerCommand(t *testing.T) {
	...
	err := guardLocalPathLinks(
		"[screenshot]("+shot+")",
		"issue description",
		"`orchestra issue update` cannot carry files — deliver the file with `orchestra issue comment add <issue-id> --attachment <path>` instead, and drop the link.",
	)
	if err == nil {
		t.Fatal("expected a hard failure")
	}
	msg := err.Error()
	if !strings.Contains(msg, "`orchestra issue update` cannot carry files") {
		t.Errorf("update hint missing, got: %v", msg)
	}
	if !strings.Contains(msg, "orchestra issue comment add <issue-id> --attachment <path>") {
		t.Errorf("update hint must redirect to comment add, got: %v", msg)
	}
	if strings.Contains(msg, "orchestra issue update --attachment") {
