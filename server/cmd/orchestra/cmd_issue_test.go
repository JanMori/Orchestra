...
func TestResolveTextFlag(t *testing.T) {
	...
	t.Run("file body is preserved verbatim with non-ASCII content", func(t *testing.T) {
		// The workdir guardrail (ISS-4252) requires the file to live inside the
	...
	// ISS-4252: a --<name>-file path that resolves outside the working
...
// TestEnsureAttachmentWithinWorkdir covers the ISS-4252 guardrail extended to
...
// TestRunIssueCommentAddRejectsExternalAttachmentWithZeroUploads is the ISS-4252
...
func TestRunIssuePullRequestsListsLinkedPRsAsJSON(t *testing.T) {
				...
				"pull_requests": []map[string]any{
					{
						...
						"number": float64(42),
						"state":  "open",
						"title":  "ISS-2818 add issue PR CLI",
					},
				},
			})
		default:
	...
	r, w, _ := os.Pipe()
	os.Stdout = w
	err := runIssuePullRequests(cmd, []string{"ISS-2818"})
	_ = w.Close()
	os.Stdout = old
	out, _ := io.ReadAll(r)
	if err != nil {
	...
	if want := []string{"/api/issues/ISS-2818", "/api/issues/issue-uuid/pull-requests"}; fmt.Sprint(gotPaths) != fmt.Sprint(want) {
	...
	if pr["url"] != "https://github.com/multica-ai/multica/pull/42" || pr["number"] != float64(42) || pr["state"] != "open" || pr["title"] != "ISS-2818 add issue PR CLI" {
...
func TestRunIssueUsageReturnsTokenSummaryAsJSON(t *testing.T) {
	...
	r, w, _ := os.Pipe()
	os.Stdout = w
	err := runIssueUsage(cmd, []string{"ISS-2818"})
	_ = w.Close()
	os.Stdout = old
	out, _ := io.ReadAll(r)
	if err != nil {
	...
	if want := []string{"/api/issues/ISS-2818", "/api/issues/issue-uuid/usage"}; fmt.Sprint(gotPaths) != fmt.Sprint(want) {
		t.Fatalf("paths = %v, want %v", gotPaths, want)
	}
	var payload map[string]any
	if err := json.Unmarshal(out, &payload); err != nil {
...
func TestRunIssuePullRequestsTableIncludesCoreFields(t *testing.T) {
	...
	out, _ := io.ReadAll(r)
	text := string(out)
	for _, want := range []string{"NUMBER", "STATE", "TITLE", "URL", "42", "open", "ISS-2818 add issue PR CLI", "https://github.com/multica-ai/multica/pull/42"} {
...
func TestFormatAssignee(t *testing.T) {
	actors := actorDisplayLookup{
		state: &actorDisplayLookupState{
			...
			crews:        map[string]string{"sq-1": "Super Human"},
			membersLoaded: true,
			agentsLoaded:  true,
			crewsLoaded:  true,
		},
	...
	tests := []struct {
		...
		{"crew", map[string]any{"assignee_type": "crew", "assignee_id": "sq-1"}, "crew:Super Human"},
...
func TestResolveIssueRef(t *testing.T) {
	...
	t.Run("identifier is resolved before prefix lookup", func(t *testing.T) {
		...
		client := cli.NewAPIClient(srv.URL, "ws-1", "test-token")
		got, err := resolveIssueRef(context.Background(), client, "ISS-1852")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if listCalled {
		...
		if got.ID != issue["id"] || got.Display != "ISS-1852" {
			t.Fatalf("got %#v", got)
		}
	})
	...
	t.Run("short UUID prefix is rejected without any HTTP call", func(t *testing.T) {
		...
		if msg := err.Error(); !strings.Contains(msg, "TASK-") {
	...
	t.Run("non-hex gibberish is rejected with key/UUID guidance", func(t *testing.T) {
		// Inputs that are neither ISS-key, full UUID, nor a hex prefix
...
func TestRunIssueRunMessagesResolvesShortTaskPrefix(t *testing.T) {
	...
	_ = cmd.Flags().Set("issue", "ISS-1852")
...
func TestResolveAssignee(t *testing.T) {
	...
	crewsResp := []map[string]any{
		{"id": "crew-4444", "name": "Super Human"},
	}
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/api/crews":
			json.NewEncoder(w).Encode(crewsResp)
		default:
			http.NotFound(w, r)
		}
	}))
	...
	// ISS-2165: crew names must resolve to (crew, <id>) so the autopilot
	// quick-create prompt can route work to a crew (e.g. "Super Human")
	// instead of falling through to "Unrecognized assignee".
	t.Run("match crew by exact name", func(t *testing.T) {
		aType, aID, err := resolveAssignee(ctx, client, "Super Human", issueAssigneeKinds)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if aType != "crew" || aID != "crew-4444" {
			t.Errorf("got (%q, %q), want (crew, crew-4444)", aType, aID)
		}
	})

	t.Run("match crew by case-insensitive substring", func(t *testing.T) {
	...
	t.Run("match crew by bare @ display name", func(t *testing.T) {
		aType, aID, err := resolveAssignee(ctx, client, "@Super Human", issueAssigneeKinds)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
...
func TestNormalizeAssigneeLookupInput(t *testing.T) {
	tests := []struct {
		...
		{
			name: "mention link",
			in:   "[@Super Human](mention://crew/ccccccc1-2222-3333-4444-555555555555)",
...
// TestResolveAssigneeRespectsKinds covers the ISS-2165 follow-up: callers
...
// be able to opt out of crew resolution. Without this, "--lead <CrewName>"
// would return (crew, ...) and the request would 500/403 server-side
...
func TestResolveAssigneeRespectsKinds(t *testing.T) {
	...
	t.Run("memberOrAgentKinds skips the /api/crews fetch entirely", func(t *testing.T) {
		before := crewsHits
		_, _, _ = resolveAssignee(ctx, client, "Alice", memberOrAgentKinds)
		if crewsHits != before {
			t.Errorf("expected memberOrAgentKinds to skip /api/crews, but it was called %d time(s)", crewsHits-before)
		}
	})

	t.Run("memberOrAgentKinds rejects a crew name with a member-or-agent-only error", func(t *testing.T) {
		_, _, err := resolveAssignee(ctx, client, "Super Human", memberOrAgentKinds)
		if err == nil {
			t.Fatal("expected resolution error for crew name under memberOrAgentKinds")
		}
		...
		if strings.Contains(err.Error(), "crew") {
			t.Errorf("error must not mention crew when crews are not allowed, got: %v", err)
		}
	})

	t.Run("memberOrAgentKinds rejects a crew UUID via the strict resolver", func(t *testing.T) {
		_, _, err := resolveAssigneeByID(ctx, client, "ccccccc1-2222-3333-4444-555555555555", memberOrAgentKinds)
		if err == nil {
			t.Fatal("expected not-found error for crew UUID under memberOrAgentKinds")
		}
		if !strings.Contains(err.Error(), "no member or agent") {
			t.Errorf("expected member-or-agent error wording, got: %v", err)
		}
	...
	t.Run("issueAssigneeKinds still resolves the same crew name (control)", func(t *testing.T) {
		aType, aID, err := resolveAssignee(ctx, client, "Super Human", issueAssigneeKinds)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if aType != "crew" || aID != "ccccccc1-2222-3333-4444-555555555555" {
			t.Errorf("got (%q, %q), want (crew, ccccccc1-...)", aType, aID)
		}
	})
}
...
func TestResolveAssigneeByIDStrict(t *testing.T) {
	...
	t.Run("full UUID resolves the right agent in a substring-collision workspace", func(t *testing.T) {
		// This is the ISS-1254 scenario: agent "J" is unreachable by name
	...
	// ISS-2165: --assignee-id <crew-uuid> must resolve to (crew, <id>) so
	// scripts that read the crew list and pin its UUID can assign work to a
	// crew in a single deterministic call.
	t.Run("UUID resolves a crew", func(t *testing.T) {
		aType, aID, err := resolveAssigneeByID(ctx, client, "ccccccc1-2222-3333-4444-555555555555", issueAssigneeKinds)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if aType != "crew" || aID != "ccccccc1-2222-3333-4444-555555555555" {
			t.Errorf("got (%q, %q), want crew Super Human", aType, aID)
		}
	})
	...
	t.Run("well-formed UUID with no matching entity errors", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "no member, agent, or crew") {
...
// for the ISS-2165 follow-up. Subscriber add/remove and project lead pass
// memberOrAgentKinds because their target schema rejects crews
// (subscriber: server/internal/handler/handler.go:414;
// project: server/migrations/034_projects.up.sql:10). Without this gating,
// `orchestra issue subscriber add --user "<CrewName>"` or
// `orchestra project create --lead "<CrewName>"` would resolve to
// (crew, ...) and surface as a 500/403 server-side instead of a clean
...
func TestPickAssigneeFromFlagsMemberOrAgentKinds(t *testing.T) {
	...
	t.Run("subscriber --user with a crew name is rejected without hitting /api/crews", func(t *testing.T) {
		before := crewsHits
		...
		if crewsHits != before {
			t.Errorf("memberOrAgentKinds must NOT fetch /api/crews, but it was called %d time(s)", crewsHits-before)
		}
	})

	t.Run("subscriber --user-id with a crew UUID is rejected", func(t *testing.T) {
		c := newCmd("user", "user-id")
...
func TestRunIssueCommentListFlagGuards(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.setup(cmd)
			err := runIssueCommentList(cmd, []string{"ISS-1"})
			if err == nil {
...
func TestRunIssueCommentList_RootsOnlyPassesThroughWithSince(t *testing.T) {
	...
	if err := runIssueCommentList(cmd, []string{"ISS-1"}); err != nil {
...
// fold default and its --full escape hatch (ISS-3555):
//
...
func TestRunIssueUpdateRejectsInvalidStatusBeforeRequest(t *testing.T) {
	...
	cmd.Flags().String("priority", "", "")
	_ = cmd.Flags().Set("status", "active")
	err := runIssueUpdate(cmd, []string{"ISS-1"})
	if err == nil {
...
func TestRunIssueUpdateSendsPosition(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/api/issues/ISS-1":
			json.NewEncoder(w).Encode(map[string]any{
				"id": "issue-1", "identifier": "ISS-1", "status": "todo",
			})
		case r.Method == http.MethodPut && r.URL.Path == "/api/issues/issue-1":
			...
			json.NewEncoder(w).Encode(map[string]any{
				"id": "issue-1", "identifier": "ISS-1", "status": "todo", "priority": "none",
			})
		default:
			http.NotFound(w, r)
		}
	...
	if err := runIssueUpdate(cmd, []string{"ISS-1"}); err != nil {
...
func TestIssueReorderTargetFlagGroup(t *testing.T) {
	...
	t.Run("two bool targets", func(t *testing.T) {
		cmd := newCmd()
		cmd.SetArgs([]string{"ISS-1", "--top", "--bottom"})
		err := cmd.Execute()
		if err == nil {
	...
	t.Run("before and after together", func(t *testing.T) {
		cmd := newCmd()
		cmd.SetArgs([]string{"ISS-1", "--before", "ISS-2", "--after", "ISS-3"})
		err := cmd.Execute()
		if err == nil {
...
func TestRunIssueReorderRejectsNoOpTargetValues(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.flag, func(t *testing.T) {
			...
			err := runIssueReorder(cmd, []string{"ISS-1"})
			if err == nil {
...
func reorderTestServer(t *testing.T, gotPosition *float64) *httptest.Server {
	...
	a := issue("a-id", "ISS-1", "todo", 1)
	b := issue("b-id", "ISS-2", "todo", 9)
	target := issue("t-id", "ISS-9", "todo", 20)
	c := issue("c-id", "ISS-3", "in_progress", 4)
	byRef := map[string]map[string]any{
		"ISS-1": a, "a-id": a,
		"ISS-2": b, "b-id": b,
		"ISS-9": target, "t-id": target,
		"ISS-3": c, "c-id": c,
	}
...
func TestRunIssueReorderComputesPosition(t *testing.T) {
	tests := []struct {
		...
		{"before another issue", "before", "ISS-2", 5}, // midpoint(a=1, b=9)
		{"after another issue", "after", "ISS-2", 10},  // moves below b (the bottom)
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := runIssueReorder(cmd, []string{"ISS-9"}); err != nil {
...
func TestRunIssueReorderRejectsCrossColumnTarget(t *testing.T) {
	...
	cmd := newIssueReorderTestCmd()
	_ = cmd.Flags().Set("before", "ISS-3") // ISS-3 lives in the in_progress column
	err := runIssueReorder(cmd, []string{"ISS-9"})
	if err == nil {
...
func TestFetchIssueColumnPaginates(t *testing.T) {
	all := []map[string]any{
		mkIssue("i1", "ISS-1", "todo", 1),
		mkIssue("i2", "ISS-2", "todo", 2),
		mkIssue("i3", "ISS-3", "todo", 3),
	}
...
func TestRunIssueReorderNoOpSkipsPut(t *testing.T) {
	...
	a := mkIssue("a-id", "ISS-1", "todo", 0)
	b := mkIssue("b-id", "ISS-2", "todo", 10)
	target := mkIssue("t-id", "ISS-9", "todo", 5)
	byRef := map[string]map[string]any{
		"ISS-1": a, "a-id": a,
		"ISS-2": b, "b-id": b,
		"ISS-9": target, "t-id": target,
...
func TestRunIssueReorderSingleItemColumnValidatesTarget(t *testing.T) {
	// The "todo" column holds only the issue being moved; ISS-3 lives in a
	...
	target := mkIssue("t-id", "ISS-9", "todo", 5)
	other := mkIssue("c-id", "ISS-3", "in_progress", 2)
	byRef := map[string]map[string]any{
		"ISS-9": target, "t-id": target,
		"ISS-3": other, "c-id": other,
	}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/api/issues" && r.Method == http.MethodGet {
...
func TestRunIssueUpdateOmitsPositionWhenUnset(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodGet && r.URL.Path == "/api/issues/ISS-1":
			json.NewEncoder(w).Encode(map[string]any{"id": "issue-1", "identifier": "ISS-1", "status": "todo"})
		case r.Method == http.MethodPut && r.URL.Path == "/api/issues/issue-1":
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
				t.Errorf("decode body: %v", err)
			}
			json.NewEncoder(w).Encode(map[string]any{"id": "issue-1", "identifier": "ISS-1", "title": "Renamed"})
		default:
			http.NotFound(w, r)
		}
	}))
...
// that ISS-5442 moved out of the runtime brief into this command's --help: the
// --recent saturation semantics (ISS-5372), the bounded two-step alternative,
...
func TestIssueCommentListHelpCarriesReadContract(t *testing.T) {
	...
	for _, want := range []string{
		...
		"--before string",
		// The saturation contract relocated from the brief (ISS-5372).
