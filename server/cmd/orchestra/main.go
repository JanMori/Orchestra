...
var rootCmd = &cobra.Command{
	Use:           "orchestra",
	Short:         "Orchestra CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Orchestra from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("orchestra {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Multica server URL (env: ORCHESTRA_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: ORCHESTRA_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: ORCHESTRA_DEBUG)")
