...
func init() {
	...
	// `orchestra agent env set <id>` — that path admits the agent owner or a
...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, ORCHESTRA_SERVER_URL env, or 'orchestra config set server_url <url>'")
	}
...
func resolveServerURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'orchestra setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		...
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set ORCHESTRA_WORKSPACE_ID env, or run 'orchestra config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --thinking-level, --service-tier, --custom-args, --mcp-config, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `orchestra agent env set <id>`)")
	}
