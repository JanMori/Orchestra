...
var issueAssignCmd = &cobra.Command{
	Use:   "assign <id>",
	Short: "Assign an issue to a member, agent, or crew",
	Args:  exactArgs(1),
	RunE:  runIssueAssign,
}
...
var issueReorderCmd = &cobra.Command{
	...
	Long: "Reposition an issue inside its current status column by computing a new\n" +
		...
		"different column, change its status first with `orchestra issue status`.",
	Args: exactArgs(1),
...
func init() {
	...
	issueListCmd.Flags().String("assignee", "", "Filter by assignee name (member, agent, or crew; fuzzy match)")
	issueListCmd.Flags().String("assignee-id", "", "Filter by assignee UUID — member, agent, or crew (mutually exclusive with --assignee)")
	...
	issueCreateCmd.Flags().String("assignee-id", "", "Assignee UUID — member, agent, or crew (mutually exclusive with --assignee)")
	...
	issueUpdateCmd.Flags().String("assignee-id", "", "New assignee UUID — member, agent, or crew (mutually exclusive with --assignee)")
	...
	// issue assign
	issueAssignCmd.Flags().String("to", "", "Assignee name (member, agent, or crew; fuzzy match)")
	issueAssignCmd.Flags().String("to-id", "", "Assignee UUID — member, agent, or crew (mutually exclusive with --to)")
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	if hasDesc {
		if err := guardLocalPathLinks(desc, "issue description",
			"Deliver the file itself with `orchestra issue create --attachment <path>` (repeatable) and drop the link."); err != nil {
			return err
		}
	...
	// before invoking the agent, the agent's `orchestra issue create` call
...
func runIssueUpdate(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("description") || cmd.Flags().Changed("description-stdin") || cmd.Flags().Changed("description-file") {
		...
		if err := guardLocalPathLinks(desc, "issue description",
			"`orchestra issue update` cannot carry files — deliver the file with `orchestra issue comment add <issue-id> --attachment <path>` instead, and drop the link."); err != nil {
			return err
		}
		body["description"] = desc
	}
...
func reorderTargetNotInColumnError(ctx context.Context, client *cli.APIClient, otherRef, issueRef resolvedID, status string) error {
	if other, err := fetchIssue(ctx, client, otherRef.ID); err == nil {
		if otherStatus := strVal(other, "status"); otherStatus != "" && otherStatus != status {
			return fmt.Errorf("issue %s is in the %q column but %s is in %q; move one with `orchestra issue status` first, or pick a target in the same column", otherRef.Display, otherStatus, issueRef.Display, status)
		}
	}
...
func runIssueCommentAdd(cmd *cobra.Command, args []string) error {
	...
	if err := guardLocalPathLinks(content, "comment body",
		"Deliver the file itself with `orchestra issue comment add <issue-id> --attachment <path>` (repeatable) and drop the link."); err != nil {
		return err
	}
...
type assigneeMatch struct {
	Type string // "member", "agent", or "crew"
	ID   string // user_id for members, agent id for agents, crew id for crews
	Name string
}
...
var (
	issueAssigneeKinds = assigneeKinds{member: true, agent: true, crew: true}
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	...
	// Search crews. The platform allows issues to be assigned to a crew
	...
	if kinds.crew {
		...
		if err := getAssigneeJSON(ctx, client, "/api/crews", &crews); err != nil {
			errs = append(errs, fmt.Errorf("fetch crews: %w", err))
		} else {
			for _, s := range crews {
				if strVal(s, "archived_at") != "" {
					continue
				}
				classify("crew", strVal(s, "id"), strVal(s, "name"))
			}
		}
	}
...
// (when allowed) crews. It is the deterministic counterpart to
// resolveAssignee: callers that already hold a UUID (e.g. agents reading IDs
// from `orchestra workspace member list --output json`) should use this instead of
...
func resolveAssigneeByID(ctx context.Context, client *cli.APIClient, id string, kinds assigneeKinds) (string, string, error) {
	...
	var crewErr error
	if kinds.crew {
		crewErr = getAssigneeJSON(ctx, client, "/api/crews", &crews)
	}
	...
	for _, pair := range []struct {
		enabled bool
		err     error
	}{{kinds.member, memberErr}, {kinds.agent, agentErr}, {kinds.crew, crewErr}} {
		if !pair.enabled {
			continue
		}
		hasFetch = true
	...
	if hasFetch && allFailed {
		return "", "", fmt.Errorf("failed to resolve assignee: %v; %v; %v", memberErr, agentErr, crewErr)
	}
