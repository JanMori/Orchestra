...
func (s *AutopilotService) AdmitAutopilotWebhookDelivery(
	...
	run, err := s.Queries.CreateAutopilotRun(ctx, db.CreateAutopilotRunParams{
		...
		CrewID:           autopilotCrewAttribution(autopilot),
...
func (s *AutopilotService) ensureWebhookCreateIssueTask(ctx context.Context, autopilot db.Autopilot, run db.AutopilotRun) error {
	...
	if autopilot.AssigneeType == "crew" {
		leader, _, err := s.resolveAutopilotLeader(ctx, autopilot)
		if err != nil {
			return fmt.Errorf("dispatch for webhook delivery: resolve crew leader: %w", err)
		}
		if _, err := s.TaskSvc.EnqueueTaskForCrewLeader(ctx, issue, leader.ID, autopilot.AssigneeID, pgtype.UUID{}); err != nil {
			return fmt.Errorf("dispatch for webhook delivery: repair crew task: %w", err)
		}
		return nil
	}
...
// as a human manually assigning the issue to that crew would.
//
// Creator on the issue is always the agent that will actually do the work
// (the resolved leader for a crew autopilot, otherwise the assignee agent
...
func (s *AutopilotService) dispatchCreateIssue(ctx context.Context, ap db.Autopilot, run *db.AutopilotRun, triggerTimezone string, actorUserID pgtype.UUID) error {
	...
	issue, err := qtx.CreateIssueWithOrigin(ctx, db.CreateIssueWithOriginParams{
		...
		// crew-assigned autopilots, the creator is the resolved leader —
	...
	*run = updatedRun
	...
	// crew autopilots, this is what triggers shouldEnqueueCrewLeaderOnAssign
	// → enqueueCrewLeaderTask — no separate crew-routing code needed here.
	...
	if ap.AssigneeType == "crew" {
		...
		if actorUserID.Valid {
			if _, err := s.TaskSvc.EnqueueTaskForCrewLeaderWithHandoff(ctx, issue, leader.ID, ap.AssigneeID, "", actorUserID); err != nil {
				return fmt.Errorf("enqueue crew leader task: %w", err)
			}
		} else if _, err := s.TaskSvc.EnqueueTaskForCrewLeader(ctx, issue, leader.ID, ap.AssigneeID, pgtype.UUID{}); err != nil {
			return fmt.Errorf("enqueue crew leader task: %w", err)
		}
...
func (s *AutopilotService) dispatchRunOnly(ctx context.Context, ap db.Autopilot, run *db.AutopilotRun, actorUserID pgtype.UUID) error {
	...
	if err != nil {
		...
		// if the row disappeared or the crew was archived between
		// admission and dispatch, that is a skip, not a failure.
		if errors.Is(err, pgx.ErrNoRows) || errors.Is(err, errCrewArchived) {
	...
	// Fail-closed invocation gate for crew autopilots (admission principal =
	// manual clicker, else creator — see autopilotAdmitInvoke).
	if ap.AssigneeType == "crew" && !s.autopilotAdmitInvoke(ctx, ap, agent, actorUserID) {
		return &errDispatchSkipped{reason: formatAdmissionReason(ap, "not allowed to invoke private crew leader"), code: dispatch.ReasonInvocationNotAllowed}
	}
...
//   - pgx.ErrNoRows / errCrewArchived (the row truly doesn't exist or is
...
func (s *AutopilotService) shouldSkipDispatch(ctx context.Context, ap db.Autopilot, actorUserID pgtype.UUID) (string, dispatch.ReasonCode, bool) {
	...
	agent, crewResolved, err := s.resolveAutopilotLeader(ctx, ap)
	if err != nil {
		...
		// dangling rows pointing at a deleted agent / archived crew.
		missing := errors.Is(err, pgx.ErrNoRows)
		archived := errors.Is(err, errCrewArchived)
		...
		switch {
		case archived:
			// Crew row exists but is archived — DeleteCrew's transfer
			...
			return "assignee crew is archived", dispatch.ReasonTargetUnavailable, true
		case missing && crewResolved:
			return "assignee crew cannot be resolved", dispatch.ReasonTargetUnavailable, true
		case missing && !crewResolved:
	...
	// agent-created autopilots are judged as workspace principals. For crew
...
// joining back to autopilot_run.crew_id.
...
// errCrewArchived signals that an autopilot's crew assignee has been
// archived. Distinct from a missing/loadable-but-failed crew so the
...
var errCrewArchived = errors.New("crew is archived")
...
// itself; for assignee_type='crew' it is the crew's leader_id. The second
// return is true when the resolver took the crew branch — callers use this
// to distinguish "failed loading an agent" from "failed loading a crew", so
// the admission gate can choose between fail-open (transient DB error on a
// known-good agent) and fail-closed (crew row gone, no point retrying).
//
// Archived crews are rejected here too: TransferCrewAutopilotsToLeader
// flips surviving autopilots to assignee_type='agent' on DeleteCrew, but
// the gate still has to fail closed for any row that slips through that
// transfer (e.g. crew archived through a code path that bypasses the
// handler) so an archived crew never produces work.
//
...
func (s *AutopilotService) resolveAutopilotLeader(ctx context.Context, ap db.Autopilot) (agent db.Agent, crewResolved bool, err error) {
	switch ap.AssigneeType {
	case "", "agent":
		agent, err = s.Queries.GetAgent(ctx, ap.AssigneeID)
		return agent, false, err
	case "crew":
		crew, err := s.Queries.GetCrew(ctx, ap.AssigneeID)
		if err != nil {
			return db.Agent{}, true, fmt.Errorf("load crew: %w", err)
		}
		if crew.ArchivedAt.Valid {
			return db.Agent{}, true, errCrewArchived
		}
		agent, err = s.Queries.GetAgent(ctx, crew.LeaderID)
		if err != nil {
			return db.Agent{}, true, fmt.Errorf("load crew leader: %w", err)
		}
...
func (s *AutopilotService) captureIssueCreatedFromAutopilot(ap db.Autopilot, run *db.AutopilotRun, issue db.Issue, leaderID pgtype.UUID) {
	...
	// the work (the resolved leader for crew autopilots) so per-agent task
...
// autopilot. For crew autopilots agent_id is best-effort the resolved
...
func (s *AutopilotService) autopilotAssigneeAnalytics(ap db.Autopilot) analytics.AutopilotAssignee {
	...
	if ap.AssigneeType == "crew" {
		assignee.CrewID = util.UUIDToString(ap.AssigneeID)
