...
func TestAutopilotCrewAttribution(t *testing.T) {
	...
	tests := []struct {
		...
		{"crew assignee returns crew id", db.Autopilot{AssigneeType: "crew", AssigneeID: id}, id},
		{"crew with invalid id returns zero", db.Autopilot{AssigneeType: "crew", AssigneeID: pgtype.UUID{}}, pgtype.UUID{}},
		{"unset type defaults to non-crew", db.Autopilot{AssigneeID: id}, pgtype.UUID{}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := autopilotCrewAttribution(tc.ap)
...
func TestErrDispatchSkippedUnwraps(t *testing.T) {
	base := &errDispatchSkipped{reason: "crew leader agent is archived"}
...
func TestResolveAutopilotLeaderSentinels(t *testing.T) {
	...
	if !errors.Is(errCrewArchived, errCrewArchived) {
		t.Fatal("errCrewArchived must satisfy errors.Is itself")
	}
	wrapped := fmt.Errorf("wrap: %w", errCrewArchived)
	if !errors.Is(wrapped, errCrewArchived) {
		t.Fatal("errCrewArchived must unwrap through fmt.Errorf")
	}
}
