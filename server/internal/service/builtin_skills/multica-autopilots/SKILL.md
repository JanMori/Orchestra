---
name: orchestra-autopilots
description: "Use when creating, updating, inspecting, triggering, or debugging a Orchestra autopilot (scheduled, webhook, or manual)."
user-invocable: false
allowed-tools: Bash(orchestra *)
---

# Orchestra Autopilots
...
```bash
orchestra autopilot list --output json
orchestra autopilot get <autopilot-id> --output json
orchestra autopilot runs <autopilot-id> --output json
```
...
- `create_issue` creates a Orchestra issue, making the run visible as issue state.
...
orchestra autopilot list --output json
orchestra autopilot get <autopilot-id> --output json
orchestra autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
orchestra autopilot update <autopilot-id> --status active|paused --output json
orchestra autopilot runs <autopilot-id> --output json
orchestra autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
orchestra autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
orchestra autopilot trigger <autopilot-id> --output json
orchestra autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `orchestra autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `orchestra autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a crew, inspect the crew: `orchestra crew get <crew-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `orchestra agent get <agent-id> --output json` and `orchestra runtime list --output json`.
