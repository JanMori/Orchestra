...
// branch of the `involves_user_id` 4-branch filter — owned agent, crew human
// member, crew canonical leader (via crew.leader_id, NOT a crew_member copy
// row), and crew agent member — plus a parallel set in a second workspace so
...
type involvesFixture struct {
	...
	crewMemberID   string // crew with userID as human member — branch (2)
	crewLeaderID   string // crew whose leader_id is an agent owned by userID — branch (3)
	crewAgentMemID string // crew with an owned-agent as crew_member row — branch (4)
	...
	otherWsCrewMember   string // crew with userID as human member, in other ws
	otherWsCrewLeader   string // crew whose leader is userID's agent (in other ws)
	otherWsCrewAgentMem string // crew with userID's agent as member (in other ws)
}
...
func setupInvolvesFixture(t *testing.T) *involvesFixture {
	...
	`, "Involves Other User", fmt.Sprintf("involves-other-%d@orchestra.local", suffix)).Scan(&otherUserID); err != nil {
	...
	// --- a crew we already have to satisfy NOT NULL leader_id ---
	leaderForMemberCrew := insertAgent(t, ctx, testWorkspaceID, runtimeID, fx.otherID,
		fmt.Sprintf("Involves Leader-for-MemberCrew %d", suffix))
	fx.crewMemberID = insertCrew(t, ctx, testWorkspaceID, leaderForMemberCrew,
		fmt.Sprintf("InvolvesCrewMember-%d", suffix))
	// Add the test user as a human member.
	if _, err := testPool.Exec(ctx, `
		INSERT INTO crew_member (crew_id, member_type, member_id) VALUES ($1, 'member', $2)
	`, fx.crewMemberID, fx.userID); err != nil {
		t.Fatalf("add crew human member: %v", err)
	}

	// --- crew with leader = our owned agent — branch (3). Critically, we do
	// NOT insert a crew_member row for the leader, so the test exercises the
	// canonical crew.leader_id path. ---
	fx.crewLeaderID = insertCrew(t, ctx, testWorkspaceID, fx.ownedAgentID,
		fmt.Sprintf("InvolvesCrewLeader-%d", suffix))

	// --- crew whose agent member is our owned agent — branch (4) ---
	leaderForAgentMemCrew := insertAgent(t, ctx, testWorkspaceID, runtimeID, fx.otherID,
		fmt.Sprintf("Involves Leader-for-AgentMemCrew %d", suffix))
	fx.crewAgentMemID = insertCrew(t, ctx, testWorkspaceID, leaderForAgentMemCrew,
		fmt.Sprintf("InvolvesCrewAgentMem-%d", suffix))
	// Use a fresh owned agent so the crew_member row is the only signal —
	...
	if _, err := testPool.Exec(ctx, `
		INSERT INTO crew_member (crew_id, member_type, member_id) VALUES ($1, 'agent', $2)
	`, fx.crewAgentMemID, branch4Agent); err != nil {
		t.Fatalf("add crew agent member: %v", err)
	}
	...
	leaderForOtherWsMemberCrew := insertAgent(t, ctx, otherWsID, otherRuntimeID, fx.otherID,
		fmt.Sprintf("OtherWs Leader-for-MemberCrew %d", suffix))
	fx.otherWsCrewMember = insertCrew(t, ctx, otherWsID, leaderForOtherWsMemberCrew,
		fmt.Sprintf("OtherWsCrewMember-%d", suffix))
	if _, err := testPool.Exec(ctx, `
		INSERT INTO crew_member (crew_id, member_type, member_id) VALUES ($1, 'member', $2)
	`, fx.otherWsCrewMember, fx.userID); err != nil {
		t.Fatalf("add other-ws crew member: %v", err)
	}

	fx.otherWsCrewLeader = insertCrew(t, ctx, otherWsID, fx.otherWsAgent,
		fmt.Sprintf("OtherWsCrewLeader-%d", suffix))

	leaderForOtherWsAgentMemCrew := insertAgent(t, ctx, otherWsID, otherRuntimeID, fx.otherID,
		fmt.Sprintf("OtherWs Leader-for-AgentMemCrew %d", suffix))
	fx.otherWsCrewAgentMem = insertCrew(t, ctx, otherWsID, leaderForOtherWsAgentMemCrew,
		fmt.Sprintf("OtherWsCrewAgentMem-%d", suffix))
	otherWsBranch4Agent := insertAgent(t, ctx, otherWsID, otherRuntimeID, fx.userID,
		fmt.Sprintf("OtherWs Branch4 Agent %d", suffix))
	if _, err := testPool.Exec(ctx, `
		INSERT INTO crew_member (crew_id, member_type, member_id) VALUES ($1, 'agent', $2)
	`, fx.otherWsCrewAgentMem, otherWsBranch4Agent); err != nil {
		t.Fatalf("add other-ws crew agent member: %v", err)
	}
...
func insertCrew(t *testing.T, ctx context.Context, workspaceID, leaderAgentID, name string) string {
	t.Helper()
	var id string
	// creator_id is loose (no FK) — reuse testUserID to keep the row valid.
	if err := testPool.QueryRow(ctx, `
		INSERT INTO crew (workspace_id, name, leader_id, creator_id)
		VALUES ($1, $2, $3, $4)
		RETURNING id
	`, workspaceID, name, leaderAgentID, testUserID).Scan(&id); err != nil {
		t.Fatalf("create crew %q: %v", name, err)
	}
	t.Cleanup(func() { testPool.Exec(context.Background(), `DELETE FROM crew WHERE id = $1`, id) })
	return id
}
...
func TestListIssues_InvolvesUserID_MatchesCrewMember(t *testing.T) {
	ctx := context.Background()
	fx := setupInvolvesFixture(t)
	wantID := insertIssueTo(t, ctx, testWorkspaceID,
		"issue assigned to a crew I'm a member of", "crew", fx.crewMemberID)
	if got := listIssuesInvolves(t, fx.userID); !containsIssueID(got, wantID) {
		t.Fatalf("branch (2) miss: human-member crew assignee not surfaced (want %s, got %v)", wantID, got)
	}
}
...
func TestListIssues_InvolvesUserID_MatchesLeaderViaCanonicalRelation(t *testing.T) {
	ctx := context.Background()
	fx := setupInvolvesFixture(t)
	// Fixture deliberately omits the crew_member leader-copy row, so this
	// can only match if the SQL reads crew.leader_id directly (branch 3).
	wantID := insertIssueTo(t, ctx, testWorkspaceID,
		"issue assigned to a crew my agent leads", "crew", fx.crewLeaderID)
	if got := listIssuesInvolves(t, fx.userID); !containsIssueID(got, wantID) {
		t.Fatalf("branch (3) miss: crew-leader-via-canonical assignee not surfaced (want %s, got %v)", wantID, got)
	}
}

func TestListIssues_InvolvesUserID_MatchesCrewAgentMember(t *testing.T) {
	ctx := context.Background()
	fx := setupInvolvesFixture(t)
	wantID := insertIssueTo(t, ctx, testWorkspaceID,
		"issue assigned to a crew my agent is a member of", "crew", fx.crewAgentMemID)
	if got := listIssuesInvolves(t, fx.userID); !containsIssueID(got, wantID) {
		t.Fatalf("branch (4) miss: crew agent-member assignee not surfaced (want %s, got %v)", wantID, got)
	}
}
...
func TestListIssues_InvolvesUserID_ExcludesOtherWorkspaceLeader(t *testing.T) {
	...
	fx := setupInvolvesFixture(t)
	issueID := insertIssueTo(t, ctx, testWorkspaceID,
		"cross-ws crew-leader assignee must not leak", "crew", fx.otherWsCrewLeader)
	if got := listIssuesInvolves(t, fx.userID); containsIssueID(got, issueID) {
		t.Fatalf("workspace isolation violated: cross-workspace crew-leader surfaced (id=%s); full result: %v",
			issueID, got)
	}
}

func TestListIssues_InvolvesUserID_ExcludesOtherWorkspaceCrewMember(t *testing.T) {
	ctx := context.Background()
	fx := setupInvolvesFixture(t)
	issueID := insertIssueTo(t, ctx, testWorkspaceID,
		"cross-ws crew-human-member assignee must not leak", "crew", fx.otherWsCrewMember)
	if got := listIssuesInvolves(t, fx.userID); containsIssueID(got, issueID) {
		t.Fatalf("workspace isolation violated: cross-workspace crew-human-member surfaced (id=%s); full result: %v",
			issueID, got)
	}
}

func TestListIssues_InvolvesUserID_ExcludesOtherWorkspaceCrewAgentMember(t *testing.T) {
	ctx := context.Background()
	fx := setupInvolvesFixture(t)
	issueID := insertIssueTo(t, ctx, testWorkspaceID,
		"cross-ws crew-agent-member assignee must not leak", "crew", fx.otherWsCrewAgentMem)
	if got := listIssuesInvolves(t, fx.userID); containsIssueID(got, issueID) {
		t.Fatalf("workspace isolation violated: cross-workspace crew-agent-member surfaced (id=%s); full result: %v",
			issueID, got)
	}
}
...
func TestListGroupedIssues_InvolvesUserID_MatchesLeaderViaCanonicalRelation(t *testing.T) {
	...
	fx := setupInvolvesFixture(t)
	wantID := insertIssueTo(t, ctx, testWorkspaceID,
		"grouped: crew my agent leads", "crew", fx.crewLeaderID)
	if got := listGroupedIssuesInvolves(t, fx.userID); !containsIssueID(got, wantID) {
		t.Fatalf("grouped branch (3) miss: crew-leader-via-canonical not surfaced (want %s, got %v)", wantID, got)
	}
}
