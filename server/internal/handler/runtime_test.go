...
func TestResolveViewingTZ(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Resolve', 'tz-resolve@orchestra.local', 'Asia/Tokyo') RETURNING id`,
	).Scan(&userID); err != nil {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email)
		 VALUES ('TZ Bare', 'tz-bare@orchestra.local') RETURNING id`,
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Bad', 'tz-bad@orchestra.local', 'Bad/Zone') RETURNING id`,
