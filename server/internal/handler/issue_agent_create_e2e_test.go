...
//	`issue create` path AND assigns it to a crew whose leader is a private
...
func TestAgentCreateOriginator_E2E_CreateAssignCrew_PrivateWorkerTriggered(t *testing.T) {
	...
	// Private crew leader L, owned by the same human H.
	...
	// The crew-leader assignment task must have been enqueued carrying H.
	...
	`, created.ID, leaderID).Scan(&leaderTaskID, &leaderOriginator); err != nil {
		t.Fatalf("load crew-leader task (expected one enqueued on create): %v", err)
	}
	if leaderOriginator != ownerH {
		t.Fatalf("crew-leader task originator = %q, want the original human H %q", leaderOriginator, ownerH)
	}
...
// TestAgentCreateOriginator_E2E_UpdateAssignCrew_HandlerGateAdmitsPrivateLeader
// locks the specific crew-leader GATE change in this PR, which the create path
...
// path, then assigns it to a private-leader crew via UpdateIssue. That assign
// routes through the handler enqueueCrewLeaderTask gate. Pre-fix the gate saw
...
func TestAgentCreateOriginator_E2E_UpdateAssignCrew_HandlerGateAdmitsPrivateLeader(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	// owner H; the fixture agent here plays the crew leader.
	...
	// Agent A assigns the issue to the private-leader crew via UpdateIssue.
	w = httptest.NewRecorder()
	...
	if w.Code != http.StatusOK {
		t.Fatalf("UpdateIssue (assign crew): expected 200, got %d: %s", w.Code, w.Body.String())
	}
