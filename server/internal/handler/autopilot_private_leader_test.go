...
// TestCreateAutopilot_CrewPrivateLeader_PlainMemberBlocked verifies that a
// plain member cannot create an autopilot assigned to a crew whose leader
// is a private agent.
func TestCreateAutopilot_CrewPrivateLeader_PlainMemberBlocked(t *testing.T) {
...
// TestUpdateAutopilot_CrewPrivateLeader_PlainMemberBlocked verifies that a
// plain member cannot update an autopilot to point at a private-leader crew.
func TestUpdateAutopilot_CrewPrivateLeader_PlainMemberBlocked(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
...
// TestCreateAutopilot_CrewPrivateLeader_OwnerAllowed verifies that a
// workspace owner CAN create an autopilot assigned to a private-leader crew.
func TestCreateAutopilot_CrewPrivateLeader_OwnerAllowed(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	// The AGENT OWNER creates the autopilot — allowed under ISS-3963 (workspace
	...
	r := newRequestAs(ownerID, "POST", "/api/autopilots?workspace_id="+testWorkspaceID, map[string]any{
		"title":          "owner creates private-leader crew ap",
		"assignee_type":  "crew",
		"assignee_id":    crewID,
		"execution_mode": "create_issue",
	})
...
// TestTriggerAutopilot_CrewPrivateLeader_OwnerCanDispatch verifies that a
// crew autopilot with private leader configured by an owner triggers
// correctly at dispatch time.
func TestTriggerAutopilot_CrewPrivateLeader_OwnerCanDispatch(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	// Create autopilot as the AGENT OWNER (ISS-3963: only owner/allow-listed
	// may invoke the private leader; workspace admin no longer bypasses).
	w := httptest.NewRecorder()
	r := newRequestAs(ownerID, "POST", "/api/autopilots?workspace_id="+testWorkspaceID, map[string]any{
		"title":          "dispatch test private leader crew",
		"assignee_type":  "crew",
		"assignee_id":    crewID,
		"execution_mode": "create_issue",
	})
	...
	t.Cleanup(func() {
		...
		testPool.Exec(context.Background(), `DELETE FROM agent_task_queue WHERE issue_id IN (SELECT id FROM issue WHERE workspace_id = $1 AND title LIKE 'dispatch test private leader crew%')`, testWorkspaceID)
		testPool.Exec(context.Background(), `DELETE FROM issue WHERE workspace_id = $1 AND title LIKE 'dispatch test private leader crew%'`, testWorkspaceID)
		testPool.Exec(context.Background(), `DELETE FROM autopilot WHERE id = $1`, ap.ID)
	})

	// Trigger AS THE OWNER — manual "run now" admits on the current clicker's
	// invoke permission (ISS-4525), so the owner (who can invoke the private
	// leader) must be the one clicking. A non-owner clicker is covered by
	// TestTriggerAutopilot_CrewPrivateLeader_NonOwnerClicker_Blocked below.
	w = httptest.NewRecorder()
...
// TestTriggerAutopilot_CrewPrivateLeader_NonOwnerClicker_Blocked pins the
// ISS-4525 fork fix: manual "run now" admits on the CURRENT clicker, not the
...
// forked.
func TestTriggerAutopilot_CrewPrivateLeader_NonOwnerClicker_Blocked(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	w := httptest.NewRecorder()
	r := newRequestAs(ownerID, "POST", "/api/autopilots?workspace_id="+testWorkspaceID, map[string]any{
		"title":          "clicker fork private leader crew",
		"assignee_type":  "crew",
		"assignee_id":    crewID,
		"execution_mode": "create_issue",
	})
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM autopilot_run WHERE autopilot_id = $1`, ap.ID)
		testPool.Exec(context.Background(), `DELETE FROM issue WHERE workspace_id = $1 AND title LIKE 'clicker fork private leader crew%'`, testWorkspaceID)
		testPool.Exec(context.Background(), `DELETE FROM autopilot WHERE id = $1`, ap.ID)
	})
...
// TestTriggerAutopilot_CrewPrivateLeader_PlainMemberCreator_Blocked verifies
// that if an autopilot pointing to a private-leader crew was somehow saved
// by a plain member (legacy data), dispatch is blocked at runtime.
func TestTriggerAutopilot_CrewPrivateLeader_PlainMemberCreator_Blocked(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, 'legacy illegal ap', 'crew', $2, 'create_issue', 'member', $3, 'active')
		RETURNING id
	`, testWorkspaceID, crewID, memberID).Scan(&apID); err != nil {
...
// TestTriggerAutopilot_RunOnly_CrewPrivateLeader_PlainMemberCreator_Blocked
...
func TestTriggerAutopilot_RunOnly_CrewPrivateLeader_PlainMemberCreator_Blocked(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	// Legacy autopilot: run_only mode, plain member creator, private-leader crew.
	var apID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO autopilot (workspace_id, title, assignee_type, assignee_id,
		                       execution_mode, created_by_type, created_by_id, status)
		VALUES ($1, 'legacy run_only illegal ap', 'crew', $2, 'run_only', 'member', $3, 'active')
		RETURNING id
	`, testWorkspaceID, crewID, memberID).Scan(&apID); err != nil {
		t.Fatalf("create autopilot: %v", err)
	}
