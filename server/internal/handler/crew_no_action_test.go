...
type runningCrewLeaderTaskFixture struct {
	IssueID          string
	LeaderID         string
	TaskID           string
...
func newRunningCrewLeaderTaskFixture(t *testing.T) runningCrewLeaderTaskFixture {
	t.Helper()
	...
	`, fx.LeaderID, runtimeID, issueID, triggerCommentID).Scan(&taskID); err != nil {
		t.Fatalf("create running crew leader task: %v", err)
	}
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM agent_task_queue WHERE id = $1`, taskID)
	})

	return runningCrewLeaderTaskFixture{
		IssueID:          issueID,
...
func recordCrewLeaderEvaluationForTask(t *testing.T, fx runningCrewLeaderTaskFixture, outcome string) {
	t.Helper()
	recordCrewLeaderEvaluationForTaskWithHeader(t, fx, outcome, fx.TaskID)
}

func recordCrewLeaderEvaluationForTaskWithHeader(t *testing.T, fx runningCrewLeaderTaskFixture, outcome, taskIDHeader string) {
	t.Helper()

	w := httptest.NewRecorder()
	r := newRequest("POST", "/api/issues/"+fx.IssueID+"/crew-evaluated", map[string]any{
		"outcome": outcome,
		"reason":  "test reason",
	})
	...
	testHandler.RecordCrewLeaderEvaluation(w, r)
	if w.Code != http.StatusCreated {
		t.Fatalf("RecordCrewLeaderEvaluation: expected 201, got %d: %s", w.Code, w.Body.String())
	}
}

func completeRunningTask(t *testing.T, fx runningCrewLeaderTaskFixture, output string) {
	t.Helper()
...
func TestCompleteTask_CrewLeaderNoActionDoesNotSynthesizeComment(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}

	fx := newRunningCrewLeaderTaskFixture(t)
	recordCrewLeaderEvaluationForTask(t, fx, "no_action")
	...
	if got := countAgentCommentsForIssue(t, fx.IssueID, fx.LeaderID); got != 0 {
		t.Fatalf("expected no crew leader comment after no_action completion, got %d", got)
	}
}

func TestCompleteTask_CrewLeaderNoActionCanonicalizesTaskID(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}

	fx := newRunningCrewLeaderTaskFixture(t)
	recordCrewLeaderEvaluationForTaskWithHeader(t, fx, "no_action", strings.ToUpper(fx.TaskID))
...
func TestCompleteTask_CrewLeaderActionStillSynthesizesComment(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}

	fx := newRunningCrewLeaderTaskFixture(t)
	recordCrewLeaderEvaluationForTask(t, fx, "action")
...
func TestCreateComment_CrewLeaderNoActionRejectsComment(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
