...
// TestCreateIssue_CrewPrivateLeader_PlainMemberBlocked verifies that a
// plain member cannot create an issue assigned to a crew whose leader is
// a private agent.
func TestCreateIssue_CrewPrivateLeader_PlainMemberBlocked(t *testing.T) {
...
// TestUpdateIssue_CrewPrivateLeader_PlainMemberBlocked verifies that a
// plain member cannot update an issue's assignee to a private-leader crew.
func TestUpdateIssue_CrewPrivateLeader_PlainMemberBlocked(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
...
// TestCreateIssue_CrewPrivateLeader_OwnerAllowed verifies that a workspace
// owner CAN assign an issue to a crew with a private leader.
func TestCreateIssue_CrewPrivateLeader_OwnerAllowed(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	r := newRequestAs(ownerID, "POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title":         "Owner assigns private-leader crew",
		"assignee_type": "crew",
		"assignee_id":   crewID,
	})
	testHandler.CreateIssue(w, r)
...
// TestComment_CrewPrivateLeader_PlainMemberNoEnqueue verifies that a plain
// member posting a comment on an issue assigned to a private-leader crew
// does NOT trigger the leader.
func TestComment_CrewPrivateLeader_PlainMemberNoEnqueue(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	// Create issue assigned to the crew as workspace owner.
	var issueID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (workspace_id, creator_type, creator_id, title, assignee_type, assignee_id)
		VALUES ($1, 'member', $2, 'private leader comment test', 'crew', $3)
		RETURNING id
...
// (triggerChildDoneAgent); agent and crew child-done now follow one path.
func TestChildDone_CrewPrivateLeader_PlainMemberWakesLeader(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
...
// shape that used to strand — the removed canEnqueueCrewLeader gate failed
// closed here — while a direct-to-leader-agent parent advanced fine. The
// leader must now be woken.
func TestChildDone_CrewPrivateLeader_AgentActorWakesLeader(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	agentID, ownerID, memberID := privateAgentTestFixture(t)
	workerAgentID := createHandlerTestAgent(t, "crew-private-leader-childdone-worker", nil)
...
// TestComment_CrewPrivateLeader_AgentActorAllowed verifies that an agent
// actor CAN explicitly trigger the private leader via a crew mention.
func TestComment_CrewPrivateLeader_AgentActorAllowed(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	agentID, ownerID, _ := privateAgentTestFixture(t)
	otherAgentID := createHandlerTestAgent(t, "crew-private-leader-agent-actor", nil)
	...
	// Create issue assigned to the crew.
	var issueID string
	if err := testPool.QueryRow(ctx, `
		INSERT INTO issue (workspace_id, creator_type, creator_id, title, assignee_type, assignee_id)
		VALUES ($1, 'member', $2, 'private leader agent actor test', 'crew', $3)
		RETURNING id
	`, testWorkspaceID, testUserID, crewID).Scan(&issueID); err != nil {
		t.Fatalf("create issue: %v", err)
	}
	...
	// Agent posts a comment with an explicit crew mention.
	w := httptest.NewRecorder()
	r := newRequest("POST", "/api/issues/"+issueID+"/comments", map[string]any{
		"content": "[@Crew](mention://crew/" + crewID + ") agent reporting in",
	})
