...
func (h *Handler) postChildDoneComment(ctx context.Context, parent, completed db.Issue, children []db.Issue, staged bool, closedStage int32, batch bool) {
	...
	if staged {
		...
		if batch {
			content = fmt.Sprintf(
				"%sAll sub-issues are complete — they just finished together in a batch update, most recently [%s](mention://issue/%s) — \"%s\". Continue the parent: synthesize the children's results and move it forward, or — if nothing remains — run `orchestra issue status %s in_review` to mark the parent ready for review.",
				mentionPrefix, identifier, childID, title, parentID,
			)
		} else {
			content = fmt.Sprintf(
				"%sAll sub-issues are complete — the last one, [%s](mention://issue/%s) — \"%s\", just finished. Continue the parent: synthesize the children's results and move it forward, or — if nothing remains — run `orchestra issue status %s in_review` to mark the parent ready for review.",
				mentionPrefix, identifier, childID, title, parentID,
			)
		}
	}
...
func (h *Handler) resolveAssigneeMentionLabel(ctx context.Context, workspaceID pgtype.UUID, assigneeType string, assigneeID pgtype.UUID) (string, bool) {
	...
	case "crew":
		...
		return sanitizeMentionLabel(crew.Name), true
	}
	return "", false
}
...
// agent or crew-leader assignees. Member assignees never reach this code
...
//   - crew parent: one EnqueueTaskForCrewLeader on the crew LEADER only.
//     Unlike a human @crew mention, this does NOT fan out to crew members
//     — child-done is a coordination signal, the leader decides whether
//     and how to wake the rest of the crew. Documented here so reviewers
...
//     parent's own leader, chosen (and permission-checked) at crew-assign
//     time, so no actor identity is threaded in — see triggerChildDoneCrew.
...
//   - NO self-trigger guard on either the agent OR the crew path. Waking the
...
//     (computeMentionedAgentCommentTriggers). The crew path used to skip a
//     same-crew or shared-leader child on the theory that the leader had
//     already observed the work through its own coordination cycle on the
//     child. That stranded the common pattern where a crew decomposes its
...
func (h *Handler) dispatchParentAssigneeTrigger(ctx context.Context, parent db.Issue, systemComment db.Comment) {
	...
	case "crew":
		h.triggerChildDoneCrew(ctx, parent, systemComment.ID)
	}
}
...
// crew path. Waking the parent agent when one of its children finishes is a
...
// triggerChildDoneCrew enqueues a leader-role task for the parent's crew
...
//     crew or by another crew sharing this leader, the leader must still be
//     woken on the PARENT to advance the next stage or wrap up. The prior
//     same-crew / shared-leader guards assumed the leader had already observed
...
//     the parent was assigned to the crew (validateAssigneePair). The agent
...
func (h *Handler) triggerChildDoneCrew(ctx context.Context, parent db.Issue, triggerCommentID pgtype.UUID) {
	crew, err := h.Queries.GetCrewInWorkspace(ctx, db.GetCrewInWorkspaceParams{
		ID:          parent.AssigneeID,
		WorkspaceID: parent.WorkspaceID,
	})
	...
	agent, err := h.Queries.GetAgent(ctx, crew.LeaderID)
	if err != nil || !agent.RuntimeID.Valid || agent.ArchivedAt.Valid {
		return
	}
	...
	if _, err := h.TaskService.EnqueueTaskForCrewLeader(ctx, parent, crew.LeaderID, crew.ID, triggerCommentID); err != nil {
		slog.Warn("child done: enqueue parent crew leader task failed",
			"error", err,
			"parent_id", uuidToString(parent.ID),
