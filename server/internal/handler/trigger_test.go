...
func TestHasAgentOrCrewMention(t *testing.T) {
	tests := []struct {
		name    string
		content string
		want    bool
		...
		{"crew", "[@Crew](mention://crew/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa) please coordinate", true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := hasAgentOrCrewMention(parseMentionsForTest(tt.content))
			if got != tt.want {
				t.Errorf("hasAgentOrCrewMention() = %v, want %v", got, tt.want)
			}
		})
	}
}
