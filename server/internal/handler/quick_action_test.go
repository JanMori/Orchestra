...
// OUTLIVED the templating feature (ISS-5465).
//
...
func TestValidateQuickActionAssignee(t *testing.T) {
	...
	if err := validateQuickActionAssignee("crew", "0f1e2d3c-4b5a-6978-8796-a5b4c3d2e1f0"); err != nil {
		t.Fatalf("crew binding must be accepted, got %v", err)
	}
...
// one-action / one-reached-party invariant (ISS-5465, review findings #3 and
// round-two #1).
//
...
// crew / all enqueue a second target, and MEMBER creates an inbox
...
func TestValidateQuickActionPromptRejectsSideEffectMentions(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"issue mention reaches nobody", "see [ISS-1](mention://issue/" + id + ")", false},
		...
		{"crew mention would enqueue a second target", "also [@Core](mention://crew/" + id + ")", true},
