...
// worker_agent_comment_wakes_crew_leader case, which uses empty
...
func TestCreateComment_WorkerAgentCommentWakesCrewLeader_MUL4015(t *testing.T) {
...
// pins the dedup behavior: when the crew leader ALREADY has a queued or
...
// The failure mode is:
//
//   - Member M owns crew leader L (private) and worker W (private).
//   - M assigns the issue to the crew → L's task carries originator=M.
...
//   - routeAssignedCrewLeaderFallback calls canInvokeAgent(L, "agent", W, "").
...
func TestCreateComment_WorkerAgentCommentWakesPrivateCrewLeader_MUL4015(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	// Issue assigned to the crew, created by M (testUserID). CreatorType=member
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO agent_task_queue (agent_id, runtime_id, issue_id, status, is_leader_task, originator_user_id, accountable_user_id, crew_id)
		VALUES ($1, $2, $3, 'running', TRUE, $4, $4, $5)
		RETURNING id
	`, leaderID, leaderRuntimeID, issueID, testUserID, crewID).Scan(&leaderTaskID); err != nil {
		t.Fatalf("seed leader task: %v", err)
	}
	...
	// leader via routeAssignedCrewLeaderFallback → canInvokeAgent(L, "agent",
