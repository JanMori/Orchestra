...
func TestDeriveCrewMemberStatus(t *testing.T) {
	...
	cases := []struct {
		...
		// runtime row or task — they should appear in the crew listing
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := deriveCrewMemberStatus(tc.archived, tc.runtimeStatus, tc.lastSeen, tc.hasActiveTask, now)
			if got != tc.want {
				t.Fatalf("deriveCrewMemberStatus = %q, want %q", got, tc.want)
			}
		})
	}
}
