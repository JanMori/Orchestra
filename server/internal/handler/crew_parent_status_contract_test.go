...
// other: the crew briefing (server-side, appended to the leader's
...
// leaderCommentRuntimeBrief renders the CLAUDE.md a crew leader receives on a
...
// TestCrewAssignedLeaderCanWrapUpOnCommentTurn covers the crew's most common
...
func TestCrewAssignedLeaderCanWrapUpOnCommentTurn(t *testing.T) {
	ctx := context.Background()
	leaderID, _ := seededLeaderAgent(t)
	crew := seedCrewForBriefing(t, leaderID, "Owning Crew", "")

	// The issue is assigned to this crew → the server grants status ownership.
	briefing := buildCrewLeaderBriefing(ctx, testHandler.Queries, crew, true)
	brief := leaderCommentRuntimeBrief(t, briefing)

	if !strings.Contains(briefing, "Own the parent issue status") {
		t.Fatalf("crew-assigned briefing must grant status ownership:\n%s", briefing)
	}
	...
	combined := briefing + "\n" + brief
	if !strings.Contains(combined, "orchestra issue status <issue-id> in_review") {
...
func TestGuestLeaderCannotChangeStatusOnCommentTurn(t *testing.T) {
	ctx := context.Background()
	leaderID, _ := seededLeaderAgent(t)
	crew := seedCrewForBriefing(t, leaderID, "Guest Crew", "")

	// The issue is assigned to someone else → no status ownership.
	briefing := buildCrewLeaderBriefing(ctx, testHandler.Queries, crew, false)
	brief := leaderCommentRuntimeBrief(t, briefing)
