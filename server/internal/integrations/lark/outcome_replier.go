...
// (e.g. https://multica.example). It comes from ORCHESTRA_APP_URL and is
// intentionally separate from ORCHESTRA_PUBLIC_URL, which is the backend/API
...
func NewLarkOutcomeReplier(cfg OutcomeReplierConfig) OutcomeReplier {
	...
	if cfg.AppURL == "" {
		log.Warn("lark outcome replier: ORCHESTRA_APP_URL not set; binding prompt CTA will not work")
	}
...
// when empty (self-host operators who haven't configured ORCHESTRA_APP_URL) the
