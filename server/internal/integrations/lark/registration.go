...
//     and an expiry. Orchestra renders the QR, the user scans it in the
...
const (
	...
	// Feishu account. The split-CTA UX (ISS-3083) rendered a QR against
...
type RegistrationConfig struct {
	...
	// telemetry can attribute installs back to Orchestra. Empty defaults
	// to "Orchestra".
	Source string
...
type BeginResult struct {
	DeviceCode string
	// QRCodeURL is the verification_uri_complete with Orchestra's `source`
...
type PollResult struct {
	...
	// a Feishu-first begin); after ISS-3083 follow-up it is symmetric,
...
// "<agent> - Orchestra" instead of Lark's auto-generated
...
func (c *RegistrationClient) Poll(ctx context.Context, domain, deviceCode string) (*PollResult, error) {
	...
	// because the split-CTA UI (ISS-3083) also begins on
...
// cannot attribute installs back to Orchestra in their analytics.
//
