...
import (
	...
	"github.com/JanMori/Orchestra/server/pkg/skillbundle"
...
// on the server's retry allowlist. (ISS-5370)
...
const (
	...
	// server-pushed daemon:pending_work hint triggers (ISS-5444). Short on
...
type terminalTaskReport struct {
	...
	// session because its rollout was not in the store (ISS-5305). The server
...
var (
	...
	// path override (ISS-3284) — a stale or mistyped override must fall back
...
type workspaceState struct {
	...
	// profile list (ISS-3332) as last seen from the server. An on-demand
...
type Daemon struct {
	...
	// (ISS-3284). Populated in registerRuntimesForWorkspace when a profile's
	...
	// runtime (ISS-5439) would otherwise be a data race.
	...
	// (ISS-5439). Guarded by skippedAgentsMu.
	...
	// login-shell probe + version detection instead of one per task (ISS-4486).
	...
	// wsRPC carries generic request/response RPCs (e.g. tasks.claim, ISS-4257)
	...
	// bounces the connection re-probes the batch route (ISS-4257).
	...
	// coalesce and rate-limit server-pushed "heartbeat now" hints (ISS-5444).
	...
	// reloadPendingReason explains why a confirmed orchestra version change hasn't
	...
	activeCodexStores     map[string]int  // per-issue Codex session store path -> live-task refcount; guards the store from GC mid-task (ISS-4424)
...
// has vanished from disk (ISS-4486).
//
...
//     version — a mismatched {old path, new version} (ISS-4486 review).
...
func (d *Daemon) healAgentPath(ctx context.Context, provider, command string) healOutcome {
	...
	// registration path applies (ISS-4486 review).
...
// (ISS-5439): that payload carries built-in runtimes only, so under the
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// Discover agent CLIs installed after startup (ISS-5439). Separate from the
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" && d.cfg.ServerBaseURL != "" {
		if token, tokenErr := cli.FetchDaemonSetupToken(d.cfg.ServerBaseURL); tokenErr == nil && token != "" {
			cfg.Token = token
			_ = cli.SaveCLIConfigForProfile(cfg, d.cfg.Profile)
		}
	}
	if cfg.Token == "" {
		loginHint := "'orchestra login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'orchestra login --profile %s'", d.cfg.Profile)
		}
...
//
// A round now serves a whole batch of workspace registrations (ISS-5225), so a
...
// found" state before probes were parallelized (ISS-5119). Only fast failures,
...
// was previously only visible in the daemon log (ISS-5439).
func (d *Daemon) probeBuiltinRuntime(ctx context.Context, name string, entry AgentEntry) (string, string, builtinProbeVerdict) {
	...
	for attempts < runtimeVersionProbeAttempts {
		...
		// (ISS-4486) so version detection — and thus staying registered/online —
...
// "no runtime found" state while the probes were still running (ISS-5119).
...
//
// Each probe still self-heals a vanished pinned path (ISS-4486) and re-detects
...
// registerRuntimesForWorkspaceBatch for each workspace (ISS-5225).
//
...
// (24 workspaces × 5 agents = 120 instead of 5, ISS-5225 / #5837). Beyond the
...
func (d *Daemon) registerRuntimesForWorkspaceBatchLocked(ctx context.Context, workspaceID string, builtins []map[string]string) (*RegisterResponse, string, error) {
	...
	// this host (ISS-3284). This is best-effort: a fetch error (e.g. an older
	...
	// the effective profile set is already current (ISS-3332). An empty string
...
//
// This exists for the CLI-discovery path (ISS-5439), which must not participate
...
// profiles (ISS-3284) and appends a runtime registration entry for each one
...
//
// Returns a content signature of the fetched profile list (ISS-3332). The
...
func (d *Daemon) appendProfileRuntimes(ctx context.Context, workspaceID string, runtimes *[]map[string]string, failedProfiles *[]map[string]string) string {
	...
	for _, profile := range resp.RuntimeProfiles {
		...
		// path override (ISS-3284, `multica runtime profile set-path`) wins
...
// runtime profile list (ISS-3332). An on-demand refresh diffs this against the
...
// in commits, matching the contract documented in RFC ISS-2414 §4.8.
...
// `orchestra repo checkout` accepts project-only URLs without an extra round
...
// runtime profile list (ISS-3332), compares its content signature against
...
// other API call is what surfaces a user-actionable "run orchestra login"
...
func (d *Daemon) syncWorkspacesFromAPI(ctx context.Context, reconcileProfiles bool) error {
	...
	// every workspace this sync has to register (ISS-5225). Probing is lazy —
...
// that heartbeat claimed (ISS-5444).
//
...
func (d *Daemon) handleModelList(ctx context.Context, rt Runtime, requestID string) {
	...
	// Self-heal a pinned executable path an in-place upgrade deleted (ISS-4486).
	...
	// picker for claude/codex (ISS-2339).
	...
	d.reportModelListResult(ctx, rt, requestID, map[string]any{
		...
		// must not persist them as this runtime's real catalog (ISS-5549).
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Orchestra Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// pollLoop runs the machine-level batch claim poller (ISS-4257): a single
...
//
// This trades the per-runtime isolation the old model gave (ISS-1744) for a
...
func (d *Daemon) handleTask(ctx context.Context, task Task, slot int) {
	...
	if err != nil {
		...
		// ISS-2946: route the bare error string through the canonical
...
func (d *Daemon) reportTaskResult(ctx context.Context, taskID string, result TaskResult, taskLog *slog.Logger) {
	...
	case "completed":
		...
		// ISS-2946: this fallback fires when a server-side complete
	...
	default:
		...
		if failureReason == "" {
			if result.Status == "cancelled" {
				...
				// ISS-2946: classify the agent's comment text so the
...
//
// Confirmed to load their context file, so deliberately absent here. ISS-5392
...
func gateResumeToReusedWorkdir(task *Task, taskCtx *execenv.TaskContextForEnv, envWorkDir string, taskLog *slog.Logger) bool {
	...
	if !reused && task.PriorSessionID != "" {
		...
		// the loss instead of silently restarting (ISS-4424). Set it on BOTH
		// carriers: the notice is rendered from `task` by BuildPrompt (ISS-5377
...
// forcing every crew-leader follow-up onto a fresh provider session. Worker
...
// terminal file raced and dropped the session (ISS-4886). Both proofs this
...
// isolation (ISS-4424) means the rollout may be missing: a migrated legacy home
...
func gateCodexResumeToRolloutPresence(task *Task, taskCtx *execenv.TaskContextForEnv, provider, codexHome string, taskLog *slog.Logger) {
	...
	// loss instead of silently restarting (ISS-4424). Set it on BOTH carriers:
	// the notice is rendered from `task` by BuildPrompt (ISS-5377 moved it out
...
// via gateCodexResumeToRolloutPresence, losing the conversation (ISS-5305). It
...
// verify and return immediately (ISS-5305).
...
func (d *Daemon) ensureTaskSkillBundles(ctx context.Context, task *Task) error {
	...
	for _, ref := range misses {
		...
		if err != nil {
			...
			// guesswork (ISS-5370): size + elapsed separate "this bundle is
...
// across dispatches. (GitHub #4505 / ISS-3650)
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	// `orchestra repo checkout` would reject project-only URLs that aren't also
	...
	// A custom runtime profile (ISS-3284) overrides the executable path: the
	...
	// codex is never launched under the previous version's policy (ISS-4486).
	...
	if customSpec, isCustom := d.customProfileLaunchForRuntime(task.RuntimeID); isCustom {
		...
		// upgrade deleted (ISS-4486). Only reached when no custom profile owns
	...
	// via `orchestra repo checkout <url>`.
	taskCtx := execenv.TaskContextForEnv{
		...
		// ISS-5305: the server sets this when a more recent Codex session was
	...
	// (ISS-4486). A custom codex runtime skips the self-heal, so resolvedVersion
	...
	// instead of silently downgrading a user's isolation opt-in (ISS-4957).
	...
	// of a long-idle issue (ISS-4424). No-op for non-Codex tasks / no stable key.
	if provider == "codex" {
	...
	// /orchestra_workspaces/{ws}/{short-id}/workdir hit FileNotFoundError in
	...
	// taxonomy as before — see ISS-2946 for the classifier contract.
	...
	// sessions (ISS-4424 isolates them). Drop the resume before the brief is
	...
	if env.LocalDirectory {
		defer func() {
			...
			// accumulates one directory layer per task — see ISS-2784.
	...
	// can call the Multica API and the local daemon (e.g. `orchestra repo checkout`).
	...
	// Ensure the orchestra CLI is on PATH inside the agent's environment.
	...
	execOpts := agent.ExecOptions{
		...
		// clears ResumeSessionID but not this (ISS-4424).
		//
		...
		// can never pay for it twice (ISS-5722).
	...
	// `orchestra issue status` / `orchestra issue comment add`, leaving issues
	// stuck in `todo`.
	//
	...
	// generation (ISS-5573). This daemon no longer has a suggestion pass to run
	...
	if shouldRetryWithFreshSession(result, task.PriorSessionID, tools, provider) {
		...
		//     because before ISS-5722 both fired and the turn carried the same
		//     paragraph twice.
	...
	// ISS-5305: withhold a Codex session whose rollout never reached the per-issue
	...
	// loss (PriorSessionResumeUnavailable, ISS-4424 transparency) even while
	...
	// as session_rollout_missing on the terminal callback (ISS-5305).
	...
	default:
		...
		if failureReason != "" {
			...
			// ISS-2946: classifyPoisonedError only matches the
			...
			// ISS-1949 offline backfill to re-classify after the
			// fact.
...
//     ISS-4910): the platform's own retry is supposed to inherit the session
...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID, codexHome string, msgSeq *atomic.Int32) (agent.Result, int32, error) {
	...
	// would be cut off here regardless of progress (ISS-3064).
		...
		for {
			...
			case msg, ok := <-session.Messages:
				...
				case agent.MessageStatus:
					...
					// without context.
					// ISS-5305: pin the resume pointer only once the session's
...
// outside the env root, so isActiveEnvRoot does not cover it (ISS-4424). If a GC
...
// reservation, closing the stat->remove race (ISS-4424).
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "ORCHESTRA_") || strings.HasPrefix(upper, "MULTICA_") {
		return true
	}
	switch upper {
