...
// ORCHESTRA_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("ORCHESTRA_SERVER_URL", DefaultServerURL)
	...
	//
	//   - ORCHESTRA_OPENCLAW_PATH: read by probe() via envOrDefault for the
	...
	// users already get with ORCHESTRA_OPENCLAW_PATH today. We achieve it with
	...
	claudeArgs, err := shellArgsFromEnv("ORCHESTRA_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("ORCHESTRA_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	codebuddyArgs, err := shellArgsFromEnv("ORCHESTRA_CODEBUDDY_ARGS")
	if err != nil {
		return Config{}, err
	}
	qwenArgs, err := shellArgsFromEnv("ORCHESTRA_QWEN_ARGS")
	if err != nil {
		return Config{}, err
	}
	qwenpawArgs, err := shellArgsFromEnv("ORCHESTRA_QWENPAW_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("ORCHESTRA_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("ORCHESTRA_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("ORCHESTRA_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("ORCHESTRA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexHandshakeTimeout, err := durationFromEnv("ORCHESTRA_CODEX_HANDSHAKE_TIMEOUT", DefaultCodexHandshakeTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// ORCHESTRA_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("ORCHESTRA_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}
	// ORCHESTRA_OPENCODE_IDLE_WATCHDOG narrows the no-message window for
	// OpenCode's streamed model responses. Zero removes the provider-specific
	// override and falls back to ORCHESTRA_AGENT_IDLE_WATCHDOG; positive values
	// cannot extend the global bound, and the global zero still disables the
	// whole mechanism.
	openCodeIdleWatchdog, err := durationFromEnv("ORCHESTRA_OPENCODE_IDLE_WATCHDOG", DefaultOpenCodeIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	// ORCHESTRA_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop; any
	// positive duration overrides DefaultAgentToolWatchdog.
	agentToolWatchdog, err := durationFromEnv("ORCHESTRA_AGENT_TOOL_WATCHDOG", DefaultAgentToolWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("ORCHESTRA_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via ORCHESTRA_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("ORCHESTRA_DAEMON_ID"))
	...
	// the user explicitly pins ORCHESTRA_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("ORCHESTRA_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("ORCHESTRA_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	...
	keepEnv := os.Getenv("ORCHESTRA_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("ORCHESTRA_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("ORCHESTRA_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("ORCHESTRA_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("ORCHESTRA_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("ORCHESTRA_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("ORCHESTRA_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcCodexSessionTTL, err := durationFromEnv("ORCHESTRA_GC_CODEX_SESSION_TTL", DefaultGCCodexSessionTTL)
	if err != nil {
		return Config{}, err
	}
	gcRepoTTL, err := durationFromEnv("ORCHESTRA_GC_REPO_TTL", DefaultGCRepoTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("ORCHESTRA_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
	...
	autoUpdateInterval, err := durationFromEnv("ORCHESTRA_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
	...
	autoReloadEnabled := boolFromEnv("ORCHESTRA_DAEMON_AUTO_RELOAD", true)
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid ORCHESTRA_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("ORCHESTRA_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("ORCHESTRA_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
func reresolveAgentCommand(cmd string) (string, bool) {
	...
	// this — an operator-pinned ORCHESTRA_*_PATH that no longer exists should
...
// before any ORCHESTRA_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom ORCHESTRA_*_PATH values never reach this
...
//
//   - The two vars touched (ORCHESTRA_OPENCLAW_PATH, OPENCLAW_STATE_DIR) are
...
func applyOpenclawOverride(oc *cli.OpenClawOverride) {
	...
	if oc.BinaryPath != "" {
		if _, set := os.LookupEnv("ORCHESTRA_OPENCLAW_PATH"); !set {
			_ = os.Setenv("ORCHESTRA_OPENCLAW_PATH", oc.BinaryPath)
		}
	}
	if oc.StateDir != "" {
