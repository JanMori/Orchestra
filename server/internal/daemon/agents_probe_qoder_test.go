...
// Qoder discovery gap reported from the desktop app (ISS-5524).
//
...
// rescuing an operator-pinned ORCHESTRA_QODER_PATH. A pinned absolute path that
...
func TestProbeAgentCLIs_QoderPinnedPathStaysHardMiss(t *testing.T) {
	...
	t.Setenv("PATH", "")
	t.Setenv("ORCHESTRA_QODER_PATH", "/nonexistent/pinned/qodercli")

	if entry, ok := probeAgentCLIs()["qoder"]; ok {
		t.Errorf("pinned-but-missing ORCHESTRA_QODER_PATH resolved to %q, want a hard miss", entry.Path)
	}
}
