...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "orchestra login") {
		t.Fatalf("401 warning must tell the user to run 'orchestra login', got: %s", out)
	}
}
...
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "orchestra login") {
		t.Fatalf("expected the actionable 'run orchestra login' hint in the WARN, got: %s", out)
	}
}
