...
// *NetworkError / *HTTPError), so command-level guidance — e.g. a `Orchestra
...
var kindMessages = map[ErrorKind][2]string{
	...
	KindNetworkDNS: {
		"Could not resolve the Orchestra server address. Check your network connection or the --server-url setting.",
		"无法解析 Orchestra 服务器地址。请检查网络连接或 --server-url 配置。",
	},
	KindNetworkRefused: {
		"Could not connect to the Orchestra server. Make sure the server address is correct and reachable.",
		"无法连接到 Orchestra 服务器。请确认服务器地址正确且网络可达。",
	},
	KindNetworkTLS: {
		"Could not establish a secure connection to the Orchestra server (TLS/certificate error). Check your system clock and CA certificates.",
		"无法与 Orchestra 服务器建立安全连接（TLS/证书错误）。请检查系统时间和 CA 证书。",
	},
	KindNetworkOffline: {
		"Could not reach the Orchestra server. Check your network connection.",
		"无法访问 Orchestra 服务器。请检查网络连接。",
	},
	KindAuthRequired: {
	...
	KindServerError: {
		"The Orchestra service is temporarily unavailable (server error). Please try again later; if it persists, contact support. Re-run with --debug to see the raw server response.",
		"Orchestra 服务暂时不可用（服务器错误）。请稍后重试；若持续出现请联系支持。可加 --debug 查看服务器原始响应。",
	},
	KindUnknown: {
...
// GH #5948), and the server-side wording added in ISS-4417 never reached them.
