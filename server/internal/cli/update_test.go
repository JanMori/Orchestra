...
func TestReleaseAssetCandidates(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			goarch:        "arm64",
			wantAssets: []string{
				"orchestra-cli-1.2.3-darwin-arm64.tar.gz",
		...
		},
		{
			...
			goarch:        "amd64",
			wantAssets: []string{
				"orchestra-cli-1.2.3-linux-amd64.tar.gz",
		...
		},
		{
			...
			goarch:        "amd64",
			wantAssets: []string{
				"orchestra-cli-1.2.3-windows-amd64.zip",
...
func TestFindReleaseAsset(t *testing.T) {
	t.Run("prefers versioned asset when both names exist", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			...
			{Name: "orchestra-cli-1.2.3-darwin-amd64.tar.gz", BrowserDownloadURL: "new"},
		}
		...
		if got.Name != "orchestra-cli-1.2.3-darwin-amd64.tar.gz" {
...
func TestFindChecksumManifestAsset(t *testing.T) {
	t.Run("finds checksums.txt among assets", func(t *testing.T) {
		assets := []GitHubReleaseAsset{
			{Name: "orchestra-cli-1.2.3-darwin-arm64.tar.gz"},
			{Name: "checksums.txt", BrowserDownloadURL: "https://example/checksums.txt"},
			{Name: "orchestra-cli-1.2.3-linux-amd64.tar.gz"},
		}
...
func TestParseChecksumManifest(t *testing.T) {
	manifest := []byte(strings.Join([]string{
		...
		"",
		"aaaa1111  orchestra-cli-1.2.3-darwin-arm64.tar.gz",
		"bbbb2222  orchestra-cli-1.2.3-darwin-amd64.tar.gz",
	...
	t.Run("returns lowercase sha for matched entry", func(t *testing.T) {
		got, err := parseChecksumManifest(manifest, "orchestra-cli-1.2.3-darwin-arm64.tar.gz")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if got != "aaaa1111" {
	...
	t.Run("skips blank lines and comments", func(t *testing.T) {
		...
		if _, err := parseChecksumManifest(manifest, "orchestra-cli-1.2.3-darwin-arm64.tar.gz"); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	})
}
