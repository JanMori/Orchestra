...
func TestCLIConfig_BackwardCompat_OldFileLoadsWithNilBackends(t *testing.T) {
	...
	cfgDir := filepath.Join(tmp, ".orchestra")
	if err := os.MkdirAll(cfgDir, 0o755); err != nil {
		t.Fatal(err)
	}
	historical := `{
  "server_url": "https://api.orchestra.ai",
  ...
  "token": "mul_abcdef"
	...
	if cfg.ServerURL != "https://api.orchestra.ai" {
...
func TestCLIConfig_BackwardCompat_NilBackendsOmittedFromJSON(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(tmp, ".orchestra", "config.json"))
	if err != nil {
		t.Fatal(err)
	}
	if string(data) == "" {
...
func TestCLIConfig_ProfileCommandOverrides_OmittedWhenEmpty(t *testing.T) {
	...
	cfg := CLIConfig{ServerURL: "https://api.orchestra.ai", Token: "mul_xyz"}
	if err := SaveCLIConfig(cfg); err != nil {
		t.Fatal(err)
	}
