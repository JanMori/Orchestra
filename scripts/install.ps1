...
function Get-InstalledCliVersion {
    try {
        $firstLine = orchestra version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Orchestra CLI from GitHub Releases..."
    ...
    try {
        ...
        $releaseAsset = "orchestra-cli-$version-windows-$arch.zip"
    ...
    Write-Ok "Orchestra CLI installed to $binDir\multica.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "multica") {
        ...
        if ($isUpToDate) {
            Write-Ok "Orchestra CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Orchestra CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Orchestra CLI upgraded ($currentVer -> $newVer)"
        return
    }
...
function Start-DefaultInstall {
    ...
    Write-Host "  [OK] Orchestra CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
...
function Start-LocalInstall {
    ...
    Write-Host ""
    Write-Host "     orchestra setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
